...
#             $foo2_fl    -> %tmp_fl_02%
    ...
    # 490 (for library 4.9.0)
    # 491 (for library 4.9.1)
...
#	$tst_cmd[2]="ncks -h -O $fl_fmt $nco_D_flg --map=".$drc_dat."/map_ne30np4_to_fv129x256_aave.20150901.nc ".$drc_dat."/ne30_tst.nc %tmp_fl_00%";
	...
	$dsc_sng="ncks regrid FSNT 2D->1D to test conservation (does rsync to dust.ess.uci.edu)";
	$tst_cmd[0]="rsync dust.ess.uci.edu:data/maps/map_fv129x256_to_ne30np4_aave.20150901.nc .";
	...
	$dsc_sng="ncks regrid FSNT 2D->2D to test conservation (does rsync to dust.ess.uci.edu)";
	$tst_cmd[0]="rsync dust.ess.uci.edu:data/maps/map_fv129x256_to_fv257x512_aave.20150901.nc .";
	$tst_cmd[1]="rsync dust.ess.uci.edu:data/ne30/rgr/ne30_2D.nc .";
...
# 20151205: using %tmp_fl_00% in arguments like "--rgr grid=%tmp_fl_00%" leads to epic fail, so hard-code filenames that are arguments
	$dsc_sng="ncks infer 129x256 FV gridfile (does rsync to dust.ess.uci.edu)";
	$tst_cmd[0]="rsync dust.ess.uci.edu:data/ne30/rgr/ne30_2D.nc .";
...
# ncap2 #19
# ncap2 -h -O -v -S ~/nco/data/ncap2_tst.nco ~/nco/data/in.nc ~/foo.nc
# ncks -C -H --trd -v nbr_err_ttl -s '%d' ~/foo.nc
   $dsc_sng="Run self-contained test script";
   $tst_cmd[0]="ncap2 -h -O $fl_fmt $nco_D_flg -v -S '../data/ncap2_tst.nco' $in_pth_arg in.nc %tmp_fl_00%";
   $tst_cmd[1]="ncks -C -H --trd -v nbr_err_ttl -s '%d' %tmp_fl_00%";
   $tst_cmd[2]="0";
   $tst_cmd[3]="SS_OK";
   NCO_bm::tst_run(\@tst_cmd);
...
# 20240409 Change from old (libnetCDF-emulating) notation to new CF convention by searching for quantization_nsd in attribute 4 instead of QuantizeBitGroomNumberOfSignificantDigits in attribute 3
...
#    $tst_cmd[1]="ncks -m --trd -v ppc_dbl %tmp_fl_00 | grep 'QuantizeBitGroomNumberOfSignificantDigits'";
    $tst_cmd[1]="ncks -m --trd -v ppc_dbl %tmp_fl_00 | grep 'quantization_nsd'";
#    $tst_cmd[2]="ppc_dbl attribute 3: quantization_nsd, size = 1 NC_INT, value = 3";
    $tst_cmd[2]="ppc_dbl attribute 4: quantization_nsd, size = 1 NC_INT, value = 3";
    $tst_cmd[3]="SS_OK";   
    NCO_bm::tst_run(\@tst_cmd);
    $#tst_cmd=0; # Reset array 			
...
# ncks -4 -O -C --baa=0 -v ppc_dbl,ppc_big --ppc .?=4 --ppc ppc_big=3 ~/nco/data/in.nc ~/foo.nc
# ncks -4 -O -C --baa=0 -v ppc_dbl,ppc_big --ppc ppc_big,ppc_dbl=4 ~/foo.nc ~/foo2.nc
# ncks -m --trd -v ppc_big ~/foo2.nc
    ...
    $tst_cmd[2]="ncks -m --trd -v ppc_big %tmp_fl_01 | grep 'quantization_nsd'";
    $tst_cmd[3]="ppc_big attribute 4: quantization_nsd, size = 1 NC_INT, value = 3";
    $tst_cmd[4]="SS_OK";   
    NCO_bm::tst_run(\@tst_cmd);
    $#tst_cmd=0; # Reset array 			
...
# ncks -m --trd -v ppc_big ~/foo2.nc
    ...
    $tst_cmd[2]="ncks -m --trd -v ppc_dbl %tmp_fl_01 | grep 'quantization_nsd'";
    $tst_cmd[3]="ppc_dbl attribute 4: quantization_nsd, size = 1 NC_INT, value = 3";
    $tst_cmd[4]="SS_OK";   
    NCO_bm::tst_run(\@tst_cmd);
    $#tst_cmd=0; # Reset array 			
