...
use vars qw(
	    ...
	    $V_flops $V_int_INS $ncwa_wallclock_sec_nowt $ncwa_wallclock_sec_wt $weighting $wgt_reuse $V_intops
...
$weighting                  = 1; # 1 for weighting, 0 for NOT weighting (no effect on ncbo)
$wgt_reuse                  = 0; # 1 for weight re-use (new default), 0 no re-use (old default)
$stl_5km                    = 0; # 1 if file is stl_5km file, 0 for ipcc file
$dim_sz                     = 1; # simple initialization for dimension size
$MRV                        = 0; # Use MRV optimization 
...
if ($stl_5km) {               # lat   lon
    ...
    $rank = @dim_vars         = (8, 32, 128, 256); # Rank vars that are >94% of file
...
if (!$ncbo) {
	...
	if($weighting == 0){ # without weighting (weight reuse value does not matter when their are no weights)
	    $K_Ia = 1; # fudge for Ia
	    $K_Mu = 1; # fudge for Mu
	    ...
	    $Ms_cnt = $dsz; # 19d
	}elsif($weighting == 1){ # with weighting
	    $K_Ia = 1; # fudge for Ia
	    $K_Mu = 1; # fudge for Mu
	...
	if($weighting == 1 && $wgt_reuse == 1){ # With weight re-use
	...
	if($MRV == 1){
	    ...
	    # fxm: fix factors
	    if($weighting = 1){$MRV_mlt_fct=2;}else{$MRV_mlt_fct=1;}
	    $Ia_cnt-=$MRV_mlt_fct*$K_Ia * ($dsz * ((22*$rank) + (5*$R_wgt) + 18 + (7/$sz_avg_blk)) + (2*$sz_wgt_blk)); # 20b
	    $Mu_cnt-=$MRV_mlt_fct* $K_Mu * ($dsz * ((12*$rank) + (3*$R_wgt) + 4 + ($bytes_per_val + 4) / $sz_avg_blk) + ($bytes_per_val*$sz_wgt_blk)); # 20c
	    $Ms_cnt-=$MRV_mlt_fct* 3*$dsz; # 20d
	} # !MRV
