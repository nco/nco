 ...
 trv_tbl_sct * const trv_tbl) /* I/O [sct] Traversal table */
{
  ...
  for(unsigned idx_tbl=0;idx_tbl<trv_tbl->nbr;idx_tbl++){
	...
	if(!nco_is_spc_in_cf_att(grp_id, "bounds", var_id, NULL) && !nco_is_spc_in_cf_att(grp_id, "climatology", var_id,NULL) && !nco_is_spc_in_cf_att(grp_id, "coordinates", var_id, NULL)){
