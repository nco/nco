 ...
 const trv_tbl_sct * const trv_tbl)  /* I [sct] GTT (Group Traversal Table) */
{
  ...
  const char fnc_nm[]="nco_aux_evl_trv()"; /* [sng] Function name */
  ...
  *lmt_nbr=0;
  ...
  for(unsigned idx_var=0;idx_var<trv_tbl->nbr;idx_var++){
    ...
    if(var_trv.nco_typ == nco_obj_typ_var && var_trv.flg_xtr){
      ...
      /* Obtain lat/lon variable names. NB: use group ID */
      has_lat_lon=nco_find_lat_lon(grp_id,var_nm_lat,var_nm_lon,&units,&lat_id,&lon_id,&crd_typ);

      if(!has_lat_lon) continue;

      /* Obtain dimension information of lat/lon coordinates NB: use group ID */
      ...
      if(nco_dbg_lvl_get() >= nco_dbg_dev){
        (void)fprintf(stdout,"%s: DEBUG %s variable <%s> (%d) limits\n",nco_prg_nm_get(),fnc_nm,
          trv_tbl->lst[idx_var].nm_fll,*lmt_nbr); 
      }
