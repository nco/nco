/* $Header: /data/zender/nco_20150216/nco/src/nco/ncra.c,v 1.535 2014-05-19 03:38:50 pvicente Exp $ */
...
main(int argc,char **argv)
{
  ...
  const char * const CVS_Id="$Id: ncra.c,v 1.535 2014-05-19 03:38:50 pvicente Exp $"; 
  const char * const CVS_Revision="$Revision: 1.535 $";
...
#pragma omp parallel for default(none) private(idx,in_id) shared(nco_dbg_lvl,fl_idx,FLG_BFR_NRM,in_id_arr,nbr_var_prc,nco_op_typ,rcd,var_prc,var_prc_out,nbr_dmn_fl,trv_tbl,var_trv,grp_id,gpe,grp_out_fll,grp_out_id,out_id,var_out_id)
    ...
    if(nco_prg_id == ncge && fl_idx==0 ){
      (void)nco_nsm_prn_wrt_att(in_id,out_id,gpe,trv_tbl);   
    }
