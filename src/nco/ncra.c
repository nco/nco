/* $Header: /data/zender/nco_20150216/nco/src/nco/ncra.c,v 1.390 2013-09-16 22:32:44 pvicente Exp $ */
...
main(int argc,char **argv)
{
  ...
  const char * const CVS_Id="$Id: ncra.c,v 1.390 2013-09-16 22:32:44 pvicente Exp $"; 
...
#pragma omp parallel for default(none) private(idx,in_id) shared(CNV_ARM,base_time_crr,base_time_srt,dbg_lvl,fl_in,fl_out,idx_rec_crr_in,idx_rec_out,rec_usd_cml,in_id_arr,REC_FRS_GRP,REC_LST_DSR,md5,nbr_var_prc,nco_op_typ,FLG_BFR_NRM,FLG_MRO,out_id,prg,rcd,var_prc,var_prc_out,nbr_dmn_fl,trv_tbl,var_trv,grp_id,gpe,grp_out_fll,grp_out_id,var_out_id,idx_rec,lmt_dmn_nbr)
        ...
        for(idx=0;idx<nbr_var_prc;idx++){
          ...
          for(int idx_dmn=0;idx_dmn<var_trv->nbr_dmn;idx_dmn++){
            ...
            if(strcmp(var_trv->var_dmn[idx_dmn].dmn_nm_fll,trv_tbl->lmt_rec[idx_rec]->nm_fll) == 0){
          ...
          for(int idx_dmn=0;idx_dmn<var_trv->nbr_dmn;idx_dmn++){
            ...
            if(strcmp(var_trv->var_dmn[idx_dmn].dmn_nm,trv_tbl->lmt_rec[idx_rec]->nm) == 0){
              /* Custom limit */
              if(flg_lmt){
                /* Case of dimension being coordinate variable */
