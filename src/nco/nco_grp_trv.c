 ...
 dmn_sct * rec_dmn)                    /* I [sct] New "record" dimension */
{
  ...
  const char fnc_nm[]="nco_prc_cmn_nm_cat()"; /* [sng] Function name */
  ...
  for(int idx=0;idx<nbr_cmn_nm;idx++){
    ...
    if(trv_1 && trv_2 && cmn_lst[idx].flg_in_fl[0] && cmn_lst[idx].flg_in_fl[1] && trv_1->flg_xtr && trv_2->flg_xtr){
      ...
      /* Process common object */
      (void)trv_tbl_prc_cat(nc_id_1,nc_id_2,nc_out_id,cnk_map,cnk_plc,cnk_sz_scl,cnk,cnk_nbr,dfl_lvl,gpe,gpe_nm,nbr_gpe_nm,CNV_CCM_CCSM_CF,(nco_bool)False,(dmn_sct **)NULL,(int)0,nco_op_typ,trv_1,trv_2,trv_tbl_1,trv_tbl_2,True,flg_def,idx_rec_out,rec_dmn);
...
(const int nc_id_1,                    /* I [id] netCDF input-file ID */
 ...
 dmn_sct *rec_dmn)                     /* I [sct] New "record" dimension */
{
  const char fnc_nm[]="trv_tbl_prc_cat()"; /* [sng] Function name */
  ...
  /* Edit group name for output */
  grp_out_fll=(char *)strdup(trv_1->grp_nm_fll);
  ...
  /* Return for non-processed variables */
  if(prc_typ_1 != prc_typ || prc_typ_2 != prc_typ) return;
  ...
  var_prc_out->nbr_dim++;
  var_prc_out->is_rec_var=True;
  var_prc_out->sz_rec=var_prc_out->sz;

  /* Allocate space to hold dimension IDs */
  var_prc_out->dim=(dmn_sct **)nco_realloc(var_prc_out->dim,var_prc_out->nbr_dim*sizeof(dmn_sct *));
  var_prc_out->dmn_id=(int *)nco_realloc(var_prc_out->dmn_id,var_prc_out->nbr_dim*sizeof(int));
  var_prc_out->cnt=(long *)nco_realloc(var_prc_out->cnt,var_prc_out->nbr_dim*sizeof(long int));
  var_prc_out->end=(long *)nco_realloc(var_prc_out->end,var_prc_out->nbr_dim*sizeof(long int));
  var_prc_out->srd=(long *)nco_realloc(var_prc_out->srd,var_prc_out->nbr_dim*sizeof(long int));
  var_prc_out->srt=(long *)nco_realloc(var_prc_out->srt,var_prc_out->nbr_dim*sizeof(long int));

  /* Moves current array by one to make room for new record dimension info */
  (void)memmove((void *)(var_prc_out->dim+1),(void *)(var_prc_out->dim),(var_prc_out->nbr_dim-1)*sizeof(dmn_sct *));
  (void)memmove((void *)(var_prc_out->dmn_id+1),(void *)(var_prc_out->dmn_id),(var_prc_out->nbr_dim-1)*sizeof(int));
  (void)memmove((void *)(var_prc_out->cnt+1L),(void *)(var_prc_out->cnt),(var_prc_out->nbr_dim-1)*sizeof(long int));
  (void)memmove((void *)(var_prc_out->end+1L),(void *)(var_prc_out->end),(var_prc_out->nbr_dim-1)*sizeof(long int));
  (void)memmove((void *)(var_prc_out->srd+1L),(void *)(var_prc_out->srd),(var_prc_out->nbr_dim-1)*sizeof(long int));
  (void)memmove((void *)(var_prc_out->srt+1L),(void *)(var_prc_out->srt),(var_prc_out->nbr_dim-1)*sizeof(long int));

  /* Insert value for new record dimension */
  var_prc_out->dim[0]=rec_dmn;
  var_prc_out->dmn_id[0]=rec_dmn->id;
  var_prc_out->cnt[0]=1L;
  var_prc_out->end[0]=-1L;
