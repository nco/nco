 ...
 const trv_tbl_sct * const trv_tbl)  /* I [sct] Traversal table */
{
  ...
  if(var_ntm_lcl > 0){
    (void)fprintf(stdout,"%s: WARNING File contains %d variables of non-atomic (i.e., compound, enum_t, opaque, vlen_t, or user-defined) type. NCO currently ignores non-atomic variables. %s will process only the atomic variables.\n",nco_prg_nm_get(),var_ntm_lcl,nco_prg_nm_get());
  } /* !var_ntm_lcl */
