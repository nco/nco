 ...
 const trv_tbl_sct * const trv_tbl)  /* I [sct] Traversal table */
{
  ...
  if(nco_prg_id_get() == ncks && nco_dbg_lvl_get() >= nco_dbg_std){
    ...
    if(nco_dbg_lvl_get() >= nco_dbg_fl) (void)fprintf(stdout,"%s: %s reports expected total RAM size of all data (not metadata) in file, accounting for subsets and hyperslabs specified in this command, is %lu B ~ %lu kB, %lu kiB ~ %lu MB, %lu MiB ~ %lu GB, %lu GiB\n",nco_prg_nm_get(),fnc_nm,(unsigned long)ram_sz_ttl,(unsigned long)round(ram_sz_ttl/NCO_BYT_PER_KB),(unsigned long)ram_sz_ttl/NCO_BYT_PER_KiB,(unsigned long)round(ram_sz_ttl/NCO_BYT_PER_MB),(unsigned long)ram_sz_ttl/NCO_BYT_PER_MiB,(unsigned long)round(ram_sz_ttl/NCO_BYT_PER_GB),(unsigned long)round(ram_sz_ttl/NCO_BYT_PER_GiB));
  } /* !ncks */
