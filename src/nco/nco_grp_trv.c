/* $Header: /data/zender/nco_20150216/nco/src/nco/nco_grp_trv.c,v 1.180 2013-04-18 21:57:06 pvicente Exp $ */
 ...
 const nco_bool flg_def)               /* I [flg] Action type (True for define variables, False when write variables ) */
{
  ...
  for(int idx=0;idx<nbr_cmn_nm;idx++){
    ...
    if(trv_1 && trv_2 && cmn_lst[idx].flg_in_fl[0] && cmn_lst[idx].flg_in_fl[1] && trv_1->flg_xtr && trv_2->flg_xtr){
      ...
      /* Process common object */
      (void)trv_tbl_prc_cat(nc_id_1,nc_id_2,nc_out_id,cnk_map,cnk_plc,cnk_sz_scl,cnk,cnk_nbr,dfl_lvl,gpe,gpe_nm,nbr_gpe_nm,CNV_CCM_CCSM_CF,(nco_bool)False,(dmn_sct **)NULL,(int)0,nco_op_typ,trv_1,trv_2,trv_tbl_1,trv_tbl_2,True,flg_def);
