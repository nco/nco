...
#define NCO_ERR_FL_LN do {						\
    ...
    (void)fprintf(stderr, "Failure in NCO function in file %s at line %d\n",__FILE__,__LINE__); \
    (void)fflush(stderr);						\
    return 2;								\
  } while (0)
...
# define NCO_VERSION_NOTE "-alpha03" /* Blank for final versions, non-blank (e.g., "-beta73") for pre-release versions */
...
#ifndef NCO_VERSION
# define NCO_VERSION "5.2.5-alpha03"
#endif /* !NCO_VERSION */
...
#define TRV_MAP_SIZE NC_MAX_DIMS
  ...
  enum nco_baa_cnv{ /* [enm] Bit-Adjustment Algorithm to use */
    ...
    nco_baa_shv, /* 1 BitShave (option since 20160117) */
    ...
    nco_baa_bgr, /* 5 BitGroomRound (option since 20200717, default 20200717--20210928) */
  ...
  typedef enum nco_wgt_typ_enm{
    ...
    nco_wgt_idw, /* Distance-weighted extrapolation */
...
typedef enum nco_xtr_typ_enm
  ...
  typedef struct{ /* rgr_sct */
    ...
    char *vrt_in; /* [sng] Name of file containing vertical grid coefficients for input file */
