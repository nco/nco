 ...
 trv_tbl_sct * const trv_tbl) /* I/O [sct] Traversal Table */
{
 ...
 skp_cf: 
  ...
  if(flg_grd_in_2D){
    ...
    else if((rcd=nco_inq_dimid_flg(in_id,"x",&dmn_id_lat)) == NC_NOERR) lat_nm_in=strdup("x");
    else{
    ...
    else if((rcd=nco_inq_dimid_flg(in_id,"y",&dmn_id_lon)) == NC_NOERR) lon_nm_in=strdup("y");
    else{
 ...
 skp_cf: 
  ...
  if(!lat_dmn_nm || !lon_dmn_nm){
    if(!lat_dmn_nm){
      ...
      else if((rcd=nco_inq_dimid_flg(in_id,"x",&dmn_id_lat)) == NC_NOERR) lat_dmn_nm=strdup("x"); /* NSIDC polar stereographic */
    ...
    if(!lon_dmn_nm){
      ...
      else if((rcd=nco_inq_dimid_flg(in_id,"y",&dmn_id_lon)) == NC_NOERR) lon_dmn_nm=strdup("y"); /* NSIDC polar stereographic */
