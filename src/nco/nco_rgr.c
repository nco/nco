...
(rgr_sct * const rgr_nfo) /* I/O [sct] Regridding structure */
{
  ...
  */
  ...
  case nco_rgr_mpf_ESMF:
    ...
    break;
  default:
    (void)fprintf(stderr,"%s: ERROR %s unable to determine map type from Conventions = %s\n",nco_prg_nm_get(),fnc_nm,att_val);
  ...
  if(nco_dbg_lvl_get() >= nco_dbg_crr){
    ...
    (void)fprintf(stderr,"src_grid_size = %li, dst_grid_size = %li, src_grid_corners = %li, dst_grid_corners = %li, src_grid_rank = %li, dst_grid_rank = %li, num_links = %li, num_wgts = %li\n",rgr_map.src_grid_size,rgr_map.dst_grid_size,rgr_map.src_grid_corners,rgr_map.dst_grid_corners,rgr_map.src_grid_rank,rgr_map.dst_grid_rank,rgr_map.num_links,rgr_map.num_wgts);
  } /* endif dbg */
