 ...
 trv_tbl_sct * const trv_tbl) /* I/O [sct] Traversal Table */
{
  ...
  if(flg_grd_out_prs){
    rcd=nco_inq_varid(tpl_id,"plev",&plev_id);
  } /* !flg_grd_out_prs */
  ...
  const int plev_id_tpl=plev_id; /* [id] Air pressure ID */
  ...
  int dmn_id_plev_in; /* [id] Dimension ID for air pressure in file to be interpolated */
  int dmn_id_plev_out; /* [id] Dimension ID for air pressure */
  ...
  if(flg_grd_out_prs){
    ...
    rcd=nco_inq_vardimid(tpl_id,plev_id,&dmn_id_plev_out);
    rcd=nco_inq_dimlen(tpl_id,dmn_id_plev_out,&plev_nbr_out);
    rcd=nco_inq_dimname(tpl_id,dmn_id_plev_out,dmn_nm);
    plev_nm_out=strdup(dmn_nm);
  } /* !flg_grd_out_prs */
  ...
  double *plev_out=NULL; /* [hPa] Air pressure on output grid */
  ...
  if(flg_grd_out_prs){
    plev_out=(double *)nco_malloc(plev_nbr_out*nco_typ_lng(var_typ_rgr));
    rcd=nco_get_var(tpl_id,plev_id,plev_out,crd_typ_out);
  } /* !flg_grd_out_prs */
  ...
  if(flg_grd_in_prs){
    rcd=nco_inq_varid(in_id,"plev",&plev_id);
  } /* !flg_grd_in_prs */
  ...
  if(flg_grd_in_prs){
    /* Interrogate plev to obtain plev dimensions */
    rcd=nco_inq_vardimid(in_id,plev_id,&dmn_id_plev_in);
    rcd=nco_inq_dimlen(in_id,dmn_id_plev_in,&plev_nbr_in);
    rcd=nco_inq_dimname(in_id,dmn_id_plev_in,dmn_nm);
    plev_nm_in=strdup(dmn_nm);
  } /* !flg_grd_in_prs */
  ...
  double *plev_in=NULL; /* [Pa] Air pressure on input grid */
  ...
  if(flg_grd_in_prs){
    plev_in=(double *)nco_malloc(plev_nbr_in*nco_typ_lng(var_typ_rgr));
    rcd=nco_get_var(in_id,plev_id,plev_in,crd_typ_out);
  } /* !flg_grd_in_prs */
  /* fxm: got to here with pure pressure grid */
	...
	for(grd_idx=0;grd_idx<grd_nbr;grd_idx++){
	  ...
	  for(out_idx=0;out_idx<out_nbr;out_idx++){
	    ...
	    if(brk_lft_idx == -1){
	      ...
	      if(xtr_LHS.xtr_vrb) (void)fprintf(fp_stdout,"%s: WARNING %s reports variable %s column %lu output value dat_out[%lu] at coordinate crd_out_mnt[%lu] = %g requires LHS extrapolation beyond leftmost valid coordinate at crd_in_mnt[%lu] = %g. Nearest valid datum is dat_in_mnt[%lu] = %g\n",nco_prg_nm_get(),fnc_nm,var_nm,grd_idx,out_idx,out_idx,crd_out_mnt[out_idx],brk_lft_idx,crd_in_mnt[brk_lft_idx],brk_lft_idx,dat_in_mnt[brk_lft_idx]);
		...
		(crd_in_mnt[brk_rgt_idx]-crd_in_mnt[brk_lft_idx]);
	      ...
	      if(xtr_RHS.xtr_vrb) (void)fprintf(fp_stdout,"%s: WARNING %s reports variable %s column %lu output value dat_out[%lu] at coordinate crd_out_mnt[%lu] = %g requires RHS extrapolation beyond rightmost valid coordinate at crd_in_mnt[%lu] = %g. Nearest valid datum is dat_in_mnt[%lu] = %g\n",nco_prg_nm_get(),fnc_nm,var_nm,grd_idx,out_idx,out_idx,crd_out_mnt[out_idx],brk_rgt_idx,crd_in_mnt[brk_rgt_idx],brk_rgt_idx,dat_in_mnt[brk_rgt_idx]);
	      // Extrapolation options are presented in decreasing order of preference
	      if(!xtr_RHS.xtr_fll){
