 ...
 trv_tbl_sct * const trv_tbl) /* I/O [sct] Traversal Table */
{
  ...
  if(rgr->fl_vrt_out) flg_vrt=True;
  assert(!flg_smf);
  assert(!flg_tps);
 ...
 const int xtn_nbr) /* [nbr] I Number of extensive variables */
{
  ...
  for(rgr_var_idx=0;rgr_var_idx<rgr_var_nbr;rgr_var_idx++){
    ...
    if(!strcmp(rgr_lst[rgr_var_idx].key,"no_stagger") || (strstr(rgr_lst[rgr_var_idx].key,"no_stg") == rgr_lst[rgr_var_idx].key)){
      rgr->flg_stg=False;
      continue;
    } /* !no_stagger */
 ...
 trv_tbl_sct * const trv_tbl) /* I/O [sct] Traversal Table */
{
  ...
  char *fl_tpl; /* [sng] Template file (output vertical grid file) */
  ...
  int tpl_id; /* [id] netCDF file ID for output vertical grid template */
  int md_open; /* [enm] Mode flag for nc_open() call */
  int out_id; /* I [id] Output netCDF file ID */
  int vrt_in_id; /* I [id] netCDF file ID for input vertical grid */
  int rcd=NC_NOERR;
  ...
  char *bd_nm; /* [sng] Ocean bottom depth (positive) field name */
  char *lt_nm; /* [sng] Layer thickness (possibly time-dependent) field name */
  char *mlc_nm; /* [sng] Name of index (1-based) to last active cell in each ocean column */
  bd_nm=(char *)strdup("bottomDepth");
  lt_nm=(char *)strdup("layerThickness");
  mlc_nm=(char *)strdup("maxLevelCell");
  ...
  if(flg_grd_out_dpt){
    ...
    if(flg_grd_out_dpt_3D){
      rcd=nco_inq_varid_flg(tpl_id,bd_nm,&bd_id);
      rcd=nco_inq_varid_flg(tpl_id,lt_nm,&lt_id);
      rcd=nco_inq_varid_flg(tpl_id,mlc_nm,&mlc_id);
      if(nco_dbg_lvl_get() >= nco_dbg_scl) (void)fprintf(stdout,"%s: DEBUG Depth output grid IDs : dpt_id = %d, bd_id = %d, lt_id = %d, mlc_id = %d\n",nco_prg_nm_get(),dpt_id,bd_id,lt_id,mlc_id);
    } /* !flg_grd_out_dpt_3D */
  } /* !flg_grd_out_dpt */
  ...
  int dmn_idx_hrz[2]; /* [idx] Horizontal dimension indices of dpt_mdp */
  ...
  int dmn_hrz_nbr_in=0; /* [nbr] Number of horizontal dimensions in input files */
  int dmn_hrz_nbr_out=0; /* [nbr] Number of horizontal dimensions in output files */
  ...
  size_t idx_dbg=rgr->idx_dbg; /* [idx] User-specifiable horizontal debugging location */
  size_t idx_fst; /* [idx] Index-offset to current surface pressure timeslice */
  size_t idx_fst_lev; /* [idx] Index-offset to current midpoint depth/height timeslice */
  size_t idx_fst_ilev; /* [idx] Index-offset to current interface depth/height timeslice */
  size_t idx_ttl; /* [idx] Total index */
  ...
  if(flg_grd_out_dpt){
	...
	for(dmn_idx=0;dmn_idx<dmn_nbr_out;dmn_idx++){
	  ...
	  if(dmn_ids_out[dmn_idx] == dmn_id_lev_out){
	    if(dmn_idx == dmn_nbr_dpt-1) flg_hrz_mrv=flg_hrz_mrv_out=False;
	    continue;
	  } /* !dmn_nbr_out */
	  ...
	  if(rec_idx == dmn_nbr_rec || dmn_nbr_out == 1){
	    ...
	    dmn_idx_hrz[dmn_hrz_nbr_out]=dmn_idx;
	    dmn_hrz_nbr_out++;
	  } /* !rec_idx, !dmn_nbr_out */
  ...
  double *bd_out=NULL; /* [m] Depth of ocean bottom (positive) on output grid */
  double *dpt_mdp_out=NULL; /* [idx] Midpoint depth/height (possibly time-dependent) on output grid */
  double *dpt_ntf_out=NULL; /* [idx] Interface depth/height (possibly time-dependent) on output grid */
  double *lt_out=NULL; /* [m] Layer thickness (possibly time-dependent) on output grid */
  int *mlc_out=NULL; /* [idx] Index (1-based) to last active cell in each column on output grid */
  ...
  if(flg_grd_out_dpt_3D){
    ...
    dpt_mdp_out=(double *)nco_malloc_dbg(tm_nbr_out*lev_nbr_out*grd_sz_out*nco_typ_lng(var_typ_rgr),fnc_nm,"Unable to malloc() dpt_mdp_out value buffer");
    mlc_out=(int *)nco_malloc(grd_sz_out*nco_typ_lng(NC_INT));
    if(dpt_id_tpl != NC_MIN_INT){
      rcd=nco_get_var(tpl_id,dpt_id_tpl,dpt_mdp_out,crd_typ_out);
      if(mlc_id_tpl != NC_MIN_INT){
	rcd=nco_get_var(tpl_id,mlc_id_tpl,mlc_out,NC_INT);
	/* Convert Fortran 1-based to C 0-based indices after input */
	for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++) mlc_out[grd_idx]--;
      }else{
	if(nco_dbg_lvl_get() >= nco_dbg_std) (void)fprintf(stderr,"%s: INFO %s Output three-dimensional depth grid file lacks bathymetry/orography-masking variable %s. Will instead construct %s from %s\n",nco_prg_nm_get(),fnc_nm,mlc_nm,mlc_nm,dpt_nm_out);
	long idx_lev_max; // [idx] Index of midpoint level with greatest depth/least height
	for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++) mlc_out[grd_idx]=NC_FILL_INT;
	for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++){
	  idx_lev_max=lev_nbr_out-1;
	  for(lev_idx=lev_nbr_out-2;lev_idx >= 0;lev_idx--){
	    if(flg_hrz_mrv){
	      if(dpt_mdp_out[lev_idx*grd_sz_out+grd_idx] != dpt_mdp_out[idx_lev_max*grd_sz_out+grd_idx])
		break;
	    }else{
	      if(dpt_mdp_out[grd_idx*lev_nbr_out+lev_idx] != dpt_mdp_out[grd_idx*lev_nbr_out+idx_lev_max])
		break;
	    } /* !flg_hrz_mrv */
	  } /* !lev_idx */
	...
	if(nco_dbg_lvl_get() >= nco_dbg_std) (void)fprintf(stderr,"%s: INFO %s Input three-dimensional depth grid file lacks bathymetry/orography-masking variable %s. Will instead construct %s from %s...\n",nco_prg_nm_get(),fnc_nm,mlc_nm,mlc_nm,lt_nm);
	has_mss_val=nco_mss_val_get_dbl(tpl_id,lt_id,&mss_val_dbl);
	if(has_mss_val) mss_val_cmp_dbl=mss_val_dbl; else mss_val_cmp_dbl=NC_FILL_DOUBLE;
	for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++) mlc_out[grd_idx]=NC_FILL_INT;
	for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++){
	  for(lev_idx=lev_nbr_out-1;lev_idx >= 0;lev_idx--){
	    if(flg_hrz_mrv) idx_ttl=lev_idx*grd_sz_out+grd_idx; else idx_ttl=grd_idx*lev_nbr_out+lev_idx;
	    if(lt_out[idx_ttl] != mss_val_cmp_dbl && lt_out[idx_ttl] > 0.0)
	      break;
	  } /* !lev_idx */
	  if(lev_idx >= 0) mlc_out[grd_idx]=lev_idx;
	} /* !grd_idx */
	
	if(nco_dbg_lvl_get() >= nco_dbg_std){
	  (void)fprintf(stdout,"%s: DEBUG %s variable %s\n",nco_prg_nm_get(),fnc_nm,"mlc_out");
	  grd_idx=idx_dbg;
	  for(lev_idx=0;lev_idx<lev_nbr_out;lev_idx++)
	    (void)fprintf(stdout,"lt_out[%lu][%lu] = %g\n",idx_dbg,lev_idx,flg_hrz_mrv ? lt_out[lev_idx*grd_sz_out+grd_idx] : lt_out[grd_idx*lev_nbr_out+lev_idx]);
	  for(grd_idx=idx_dbg;grd_idx<idx_dbg+10;grd_idx++)
	    (void)fprintf(stdout,"mlc_out[%lu] = %d\n",grd_idx,mlc_out[grd_idx]);
	} /* !dbg */
      ...
      double *z_ntf=NULL; /* [m] Depth of layer lower interfaces */
      double *z_sfc=NULL; /* [m] Ocean surface height */
      lt_sum=(double *)nco_malloc(tm_nbr_out*grd_sz_out*nco_typ_lng(var_typ_rgr));
      lt_cum_sum=(double *)nco_malloc_dbg(tm_nbr_out*grd_sz_out*lev_nbr_out*nco_typ_lng(var_typ_rgr),fnc_nm,"Unable to malloc() lt_cum_sum value buffer");
      z_ntf=(double *)nco_malloc_dbg(tm_nbr_out*grd_sz_out*lev_nbr_out*nco_typ_lng(var_typ_rgr),fnc_nm,"Unable to malloc() z_ntf value buffer");
      z_sfc=(double *)nco_malloc(tm_nbr_out*grd_sz_out*nco_typ_lng(var_typ_rgr));

      /* Initialization */
      const size_t tm_hrz_sz=tm_nbr*grd_sz_out; /* [nbr] Size of time-varying surface field */
      long lev_idx_max; /* [idx] Fortran 1-based index of midpoint level with greatest depth/least height */
      memcpy(lt_cum_sum,lt_out,tm_nbr*grd_sz_out*lev_nbr_out*nco_typ_lng(var_typ_rgr));
      for(size_t idx_out=0;idx_out<tm_hrz_sz;idx_out++) lt_sum[idx_out]=0.0;
      /* Column thickness */
      for(tm_idx=0;tm_idx<tm_nbr;tm_idx++){
	idx_fst=tm_idx*grd_sz_out;
	idx_fst_lev=tm_idx*grd_sz_out*lev_nbr_out;
	for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++){
	  lev_idx_max=mlc_out[grd_idx];
	  for(lev_idx=0;lev_idx<=lev_idx_max;lev_idx++){
	    if(flg_hrz_mrv) idx_ttl=idx_fst_lev+lev_idx*grd_sz_out+grd_idx; else idx_ttl=idx_fst_lev+grd_idx*lev_nbr_out+lev_idx;
	    if(lt_out[idx_ttl] != mss_val_cmp_dbl) lt_sum[idx_fst+grd_idx]+=lt_out[idx_ttl];
	  } /* !lev_idx */
	} /* !grd_idx */
      } /* !tm_idx */
	...
	for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++){
	  ...
	  for(lev_idx=1;lev_idx<=lev_idx_max;lev_idx++){ // NB: Starts at 1
	    if(flg_hrz_mrv){
	      idx_ttl=idx_fst_lev+lev_idx*grd_sz_out+grd_idx;
	      if(lt_cum_sum[idx_ttl] != mss_val_cmp_dbl) lt_cum_sum[idx_ttl]+=lt_cum_sum[idx_fst_lev+(lev_idx-1)*grd_sz_out+grd_idx];
	    }else{
	      idx_ttl=idx_fst_lev+grd_idx*lev_nbr_out+lev_idx;
	      if(lt_cum_sum[idx_ttl] != mss_val_cmp_dbl) lt_cum_sum[idx_ttl]+=lt_cum_sum[idx_ttl-1];
	    } /* !flg_hrz_mrv */
	  } /* !lev_idx */
	} /* !grd_idx */
      } /* !tm_idx */	
	...
	idx_fst=tm_idx*grd_sz_out;
	for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++){
	  if(lt_sum[idx_fst+grd_idx] != mss_val_cmp_dbl && bd_out[grd_idx] != mss_val_cmp_dbl) z_sfc[idx_fst+grd_idx]=lt_sum[idx_fst+grd_idx]-bd_out[grd_idx];
	} /* !grd_idx */
      } /* !tm_idx */	
	/* Interface depths */
      for(tm_idx=0;tm_idx<tm_nbr;tm_idx++){
	...
	for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++){
	  ...
	  for(lev_idx=0;lev_idx<=lev_idx_max;lev_idx++){
	    if(flg_hrz_mrv) idx_ttl=idx_fst_lev+lev_idx*grd_sz_out+grd_idx; else idx_ttl=idx_fst_lev+grd_idx*lev_nbr_out+lev_idx;
	    if(z_sfc[idx_fst+grd_idx] != mss_val_cmp_dbl && lt_cum_sum[idx_ttl] != mss_val_cmp_dbl) z_ntf[idx_ttl]=z_sfc[idx_fst+grd_idx]-lt_cum_sum[idx_ttl];
	  } /* !lev_idx */
	} /* !grd_idx */
      } /* !tm_idx */		
	/* Construction... */
      (void)memset(dpt_mdp_out,0,tm_nbr_out*lev_nbr_out*grd_sz_out*nco_typ_lng(var_typ_rgr));
      for(tm_idx=0;tm_idx<tm_nbr_out;tm_idx++){
	idx_fst_lev=tm_idx*grd_sz_out*lev_nbr_out;
	for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++){
	  lev_idx_max=mlc_out[grd_idx];
	  if(lev_idx_max >= 0){
	    for(lev_idx=0;lev_idx<=lev_idx_max;lev_idx++){
	      if(flg_hrz_mrv) idx_ttl=idx_fst_lev+lev_idx*grd_sz_out+grd_idx; else idx_ttl=idx_fst_lev+grd_idx*lev_nbr_out+lev_idx;
	      if(z_ntf[idx_ttl] != mss_val_cmp_dbl && lt_out[idx_ttl] != mss_val_cmp_dbl) dpt_mdp_out[idx_ttl]=z_ntf[idx_ttl]+0.5*lt_out[idx_ttl];
	    } /* !lev_idx */
	      /* Repeat deepest valid depth beneath bathymetry */
	    for(lev_idx=lev_idx_max+1;lev_idx<lev_nbr_out;lev_idx++){
	      if(flg_hrz_mrv){
		idx_ttl=idx_fst_lev+lev_idx*grd_sz_out+grd_idx;
		dpt_mdp_out[idx_ttl]=dpt_mdp_out[idx_fst_lev+lev_idx_max*grd_sz_out+grd_idx];
	      }else{
		idx_ttl=idx_fst_lev+grd_idx*lev_nbr_out+lev_idx;
		dpt_mdp_out[idx_ttl]=dpt_mdp_out[idx_fst_lev+grd_idx*lev_nbr_out+lev_idx_max];
	      } /* !flg_hrz_mrv */
	    } /* !lev_idx */
	  } /* !lev_idx_max */
	} /* !grd_idx */
  ...
  /* Assume input vertical grid is in input data file unless separate input vertical grid file specified */
  vrt_in_id=in_id;
  if(rgr->fl_vrt_in) fl_vrt_in=(char *)strdup(rgr->fl_vrt_in);
  if(fl_vrt_in){
    /* Make sure file is on local system and is readable or die trying */
    fl_vrt_in=nco_fl_mk_lcl(fl_vrt_in,fl_pth_lcl,HPSS_TRY,&FL_RTR_RMT_LCN);
    /* Open file using appropriate buffer size hints and verbosity */
    if(RAM_OPEN) md_open=NC_NOWRITE|NC_DISKLESS; else md_open=NC_NOWRITE;
    if(SHARE_OPEN) md_open=md_open|NC_SHARE;
    rcd+=nco_fl_open(fl_vrt_in,md_open,&bfr_sz_hnt,&vrt_in_id);
  } /* !fl_vrt_in */
  ...
  if(flg_grd_in_hyb){
    ...
    dpt_id=NC_MIN_INT;
    if(flg_grd_in_dpt_1D) rcd=nco_xtr_var_get(&fl_xtr_id,&lev_nm_in,&lev_nm_out,&rgr->lev_nm_out,&dpt_id);
    else if(flg_grd_in_dpt_3D) rcd=nco_xtr_var_get(&fl_xtr_id,&dpt_nm_in,&dpt_nm_out,&rgr->dpt_nm_out,&dpt_id);
    else assert(1 == 0);
    if(rcd != NC_NOERR){
  ...
  if(flg_grd_in_dpt){
    ...
    /* Optionally retrieve bottomDepth, maxLevelCell, layerThickness IDs */
    rcd=nco_inq_varid_flg(vrt_in_id,bd_nm,&bd_id);
    rcd=nco_inq_varid_flg(vrt_in_id,lt_nm,&lt_id);
    rcd=nco_inq_varid_flg(vrt_in_id,mlc_nm,&mlc_id);
    if(nco_dbg_lvl_get() >= nco_dbg_scl) (void)fprintf(stdout,"%s: DEBUG Depth input grid IDs : dpt_id = %d, bd_id = %d, lt_id = %d, mlc_id = %d\n",nco_prg_nm_get(),dpt_id,bd_id,lt_id,mlc_id);
  } /* !flg_grd_in_dpt */
  const int mlc_id_in=mlc_id; /* [id] Index (1-based) to last active cell in each column ID */
  ...
  if(flg_grd_in_hyb){
    ...
    if(flg_grd_hyb_cameam){
      ...
      if(lev_id_tpl == NC_MIN_INT) rcd=nco_inq_varid_flg(vrt_in_id,"lev",&lev_id);
    } /* !flg_grd_hyb_cameam */
  ...
  if(flg_grd_in_dpt){
    ...
    if(dmn_nbr_in == 1 ) assert(flg_grd_in_dpt_1D); else assert(flg_grd_in_dpt_3D);
  } /* !flg_grd_in_dpt */
  ...
  if(flg_grd_in_dpt){
	...
	for(dmn_idx=0;dmn_idx<dmn_nbr_in;dmn_idx++)
	  if(dmn_ids_in[dmn_idx] == dmn_id_lev_in)
	    break;
	//char tmp_nm[NC_MAX_NAME+1L];
	//rcd=nco_inq_varname(in_id,var_idx,tmp_nm);
	//(void)fprintf(stdout,"%s: DEBUG %s trying to infer horizontal grid from variable %s\n",nco_prg_nm_get(),fnc_nm,tmp_nm);
	/* Does current variable have lev dimension? */
	if(dmn_idx < dmn_nbr_in){
	  /* Assume all non-record dimensions larger than lev are horizontal spatial dimensions */
	  //(void)fprintf(stdout,"%s: DEBUG %s variable %s is multi-dimensional and includes dmn_id_lev_in\n",nco_prg_nm_get(),fnc_nm,tmp_nm);
	  for(dmn_idx=0;dmn_idx<dmn_nbr_in;dmn_idx++){
	  ...
	  /* If inner dimension loop completed, then horizontal information retrieved, no need to examine another variable */
	  if(dmn_idx == dmn_nbr_in){
	    if(nco_dbg_lvl_get() >= nco_dbg_std){
      ...
      if(nco_dbg_lvl_get() >= nco_dbg_std)
	if(var_idx == var_nbr)
	  (void)fprintf(stdout,"%s: INFO %s reports 1D input file has no variables suitable for inferring horizontal grid, i.e., input could be vertical only, with no horizontal extent\n",nco_prg_nm_get(),fnc_nm);
    } /* !flg_grd_in_dpt_1D */
    if(flg_grd_in_dpt_1D && flg_grd_out_dpt_1D) grd_sz_out=grd_sz_in;
  } /* flg_grd_in_dpt */
  ...
  if(flg_grd_in_prs){
	...
	if(dmn_idx < dmn_nbr_in){
	  ...
	  if(dmn_idx < dmn_nbr_in-1){
	    /* Yes (flg_mrv_hrz == True). Assume remaining dimensions are horizontal spatial dimensions */
	    ...
	    /* No (flg_mrv_hrz == False). Assume preceding one or two (how to decide?) dimensions are horizontal spatial dimensions */
	    if(dmn_nbr_in > 1){
	      (void)fprintf(stdout,"%s: WARNING %s reports prs->prs vertical interpolation on multi-dimensional variable whose vertical dimension is most-rapidly-varying (MRV, flg_mrv_hrz == False). This feature is only supported for unstructured grids (i.e., grids with one horizontal dimension). Ask the NCO project to work on this for logically 2D horizontal grids. The problem is that automatically determining whether the input dataset is unstructured or is logically 2D takes an enormous effort in the general case. Might be most robust and easiest to add a user-invokable flag, e.g., --input_2d, that overrides the default (unstructured grid) assumption. Just thinking out loud. Never mind me, move along...\n",nco_prg_nm_get(),fnc_nm);
	      /* 20230405 fxm: Following block works for unstructured grids, is borken for logically 2D grids */
	      for(int dmn_idx_hrz=dmn_nbr_in-2;dmn_idx_hrz<dmn_nbr_in-1;dmn_idx_hrz++){
		rcd=nco_inq_dimlen(in_id,dmn_ids_in[dmn_idx_hrz],dmn_cnt_in+dmn_idx_hrz);
		grd_sz_in*=dmn_cnt_in[dmn_idx_hrz];
	      } /* !dmn_idx_hrz */
	    } /* !dmn_nbr_in */
	  ...
	  for(dmn_idx=0;dmn_idx<dmn_nbr_in;dmn_idx++){
	    ...
	    if(rec_idx != dmn_nbr_rec && dmn_nbr_in > 1 && dmn_cnt_in[dmn_idx] > 1L){
	      ...
	      /* Set tm_nbr_in for diagnostic purposes
		 Pure pressure and 1D-depth grids are assumed to be constant in time
		 Thus, leave tm_nbr == 1 (do not set tm_nbr:=tm_nbr_in) */
	      tm_nbr_in=dmn_cnt_in[dmn_idx_tm_in];
	    } /* !rec_idx, !dmn_nbr_out, !dmn_cnt_out */
	  } /* !dmn_idx */

	  /* We have gleaned both spatial and temporal grid information from this variable, no further variables need be examined */
	  break;
	} /* !dmn_idx */
      } /* !var_idx */
      if(nco_dbg_lvl_get() >= nco_dbg_std)
  ...
  if(flg_grd_in_dpt_3D || flg_grd_out_dpt_3D){
    ...
    /* Recall that if dpt_id_in == NC_MIN_INT then dpt_id := lt_id */
	...
	/* 20190330: Allow possibility that Zmid has time dimension > 1 
	   ...
	   Corner-case exception when Zmid spatial dimension on unstructured grid is unlimited */
	if(dmn_ids_in[dmn_idx] == dmn_id_lev_in){
	  if(dmn_idx == dmn_nbr_dpt-1) flg_hrz_mrv=flg_hrz_mrv_in=False;
	  continue;
	} /* !dmn_nbr_out */
	for(rec_idx=0;rec_idx<dmn_nbr_rec;rec_idx++)
	  if(dmn_ids_in[dmn_idx] == dmn_ids_rec[rec_idx])
	...
	dmn_srt[dmn_idx]=0L;
      ...
      /* 20221102: Why copy input IDs to output arrays, you ask? Because these output arrays are only used to define Zmid and mlc_out in output*/
  ...
  if(flg_grd_in_dpt){
	...
	if(mlc_id_in == NC_MIN_INT){
	  if(nco_dbg_lvl_get() >= nco_dbg_std) (void)fprintf(stderr,"%s: INFO %s Input three-dimensional depth grid file lacks bathymetry/orography-masking variable %s. Will instead construct %s from %s...\n",nco_prg_nm_get(),fnc_nm,mlc_nm,mlc_nm,dpt_nm_in);
	  long idx_lev_max; // [idx] C 0-based index of midpoint level with greatest depth/least height
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++) mlc_in[grd_idx]=NC_FILL_INT;
	...
	/* If ready-made depth is unavailable, construct it from bottomDepth and layerThickness */
	if(nco_dbg_lvl_get() >= nco_dbg_std) (void)fprintf(stderr,"%s: INFO %s Constructing %s from %s and %s\n",nco_prg_nm_get(),fnc_nm,dpt_nm_in,lt_nm,bd_nm);	
	...
	/* 20221215: Horizontal regridding omits extensive variable maxLevelCell */
	if(mlc_id_in != NC_MIN_INT){
	  rcd=nco_get_var(vrt_in_id,mlc_id_in,mlc_in,NC_INT);
	  /* Convert Fortran 1-based to C 0-based indices after input */
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++) mlc_in[grd_idx]--;
	}else{ /* !mlc_id_in */
	  if(nco_dbg_lvl_get() >= nco_dbg_std) (void)fprintf(stderr,"%s: INFO %s Input three-dimensional depth grid file lacks bathymetry/orography-masking variable %s. Will instead construct %s from %s...\n",nco_prg_nm_get(),fnc_nm,mlc_nm,mlc_nm,lt_nm);
	  has_mss_val=nco_mss_val_get_dbl(fl_xtr_id,lt_id,&mss_val_dbl);
	  if(has_mss_val) mss_val_cmp_dbl=mss_val_dbl; else mss_val_cmp_dbl=NC_FILL_DOUBLE;
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++) mlc_in[grd_idx]=NC_FILL_INT;
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++){
	    for(lev_idx=lev_nbr_in-1;lev_idx >= 0;lev_idx--){
	      if(flg_hrz_mrv) idx_ttl=lev_idx*grd_sz_in+grd_idx; else idx_ttl=grd_idx*lev_nbr_in+lev_idx;
	      if(lt_in[idx_ttl] != mss_val_cmp_dbl && lt_in[idx_ttl] > 0.0)
		break;
	    } /* !lev_idx */
	    if(lev_idx >= 0) mlc_in[grd_idx]=lev_idx;
	  } /* !grd_idx */

	  if(nco_dbg_lvl_get() >= nco_dbg_std && 0){
	    (void)fprintf(stdout,"%s: DEBUG %s variable %s\n",nco_prg_nm_get(),fnc_nm,"mlc_in");
	    grd_idx=idx_dbg;
	    for(lev_idx=0;lev_idx<lev_nbr_in;lev_idx++)
	      (void)fprintf(stdout,"lt_in[%lu][%lu] = %g\n",idx_dbg,lev_idx,flg_hrz_mrv ? lt_in[lev_idx*grd_sz_in+grd_idx] : lt_in[grd_idx*lev_nbr_in+lev_idx]);
	    for(grd_idx=idx_dbg;grd_idx<idx_dbg+10;grd_idx++)
	      (void)fprintf(stdout,"mlc_in[%lu] = %d\n",grd_idx,mlc_in[grd_idx]);
	  } /* !dbg */
	...
	for(tm_idx=0;tm_idx<tm_nbr;tm_idx++){
	  ...
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++){
	    ...
	    for(lev_idx=0;lev_idx<=lev_idx_max;lev_idx++){
	      ...
	      if(lt_in[idx_ttl] != mss_val_cmp_dbl) lt_sum[idx_fst+grd_idx]+=lt_in[idx_ttl];
	    } /* !lev_idx */
	  } /* !grd_idx */
	} /* !tm_idx */	
	/* Column cumulative thickness */
	for(tm_idx=0;tm_idx<tm_nbr;tm_idx++){
	  ...
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++){
		...
		if(lt_cum_sum[idx_ttl] != mss_val_cmp_dbl) lt_cum_sum[idx_ttl]+=lt_cum_sum[idx_fst_lev+(lev_idx-1)*grd_sz_in+grd_idx];
	      }else{
	...
	for(tm_idx=0;tm_idx<tm_nbr;tm_idx++){
	  idx_fst=tm_idx*grd_sz_in;
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++){
	    if(lt_sum[idx_fst+grd_idx] != mss_val_cmp_dbl && bd_in[grd_idx] != mss_val_cmp_dbl) z_sfc[idx_fst+grd_idx]=lt_sum[idx_fst+grd_idx]-bd_in[grd_idx];
	  } /* !grd_idx */
	} /* !tm_idx */	
	/* Interface depths */
	for(tm_idx=0;tm_idx<tm_nbr;tm_idx++){
	...
	/* Construction... */
	(void)memset(dpt_mdp_in,0,tm_nbr_in*lev_nbr_in*grd_sz_in*nco_typ_lng(var_typ_rgr));
	for(tm_idx=0;tm_idx<tm_nbr_in;tm_idx++){
	  ...
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++){
	    ...
	    if(lev_idx_max >= 0){
	      for(lev_idx=0;lev_idx<=lev_idx_max;lev_idx++){
		if(flg_hrz_mrv) idx_ttl=idx_fst_lev+lev_idx*grd_sz_in+grd_idx; else idx_ttl=idx_fst_lev+grd_idx*lev_nbr_in+lev_idx;
		if(z_ntf[idx_ttl] != mss_val_cmp_dbl && lt_in[idx_ttl] != mss_val_cmp_dbl) dpt_mdp_in[idx_ttl]=z_ntf[idx_ttl]+0.5*lt_in[idx_ttl];
	      } /* !lev_idx */
	      /* Repeat deepest valid depth beneath bathymetry */
  ...
  if(flg_grd_in_hyb){
    ...
    rcd=nco_get_var(vrt_in_id,hyai_id,hyai_in,crd_typ_out);
  ...
  if(xtr_mth == nco_xtr_fll_msv){
	...
	for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++){
	  if(ncr_idx_in){
	    ...
	    idx_lev_max=0L;
	    idx_lev_min=mlc_in[grd_idx];
	  } /* !ncr_idx_in */	
	  if(flg_hrz_mrv){
	    prs_max_in[grd_idx+idx_fst]=dpt_mdp_in[tm_idx*idx_lev_max+grd_idx];
	    prs_min_in[grd_idx+idx_fst]=dpt_mdp_in[tm_idx*idx_lev_min+grd_idx];
	  }else{
	    prs_max_in[grd_idx+idx_fst]=dpt_mdp_in[idx_fst*lev_nbr_in+idx_lev_max];
	    prs_min_in[grd_idx+idx_fst]=dpt_mdp_in[idx_fst*lev_nbr_in+idx_lev_min];
	  } /* !flg_hrz_mrv */
	} /* !grd_idx */
      } /* !tm_idx */
    } /* !flg_grd_in_dpt_3D */
    ...
    if(flg_grd_in_hyb){
	 ...
	 (CAM/EAM vertical coordinate decreases in geometric space since pressure decreases with height) */
      ...
      prs_max_in[1]=p0_in*hyam_in[1]+ps_in[0]*hybm_in[1];
      if(prs_max_in[1]-prs_max_in[0] > 0.0) ncr_idx_in=True; else ncr_idx_in=False;
      if(ncr_idx_in){
	idx_lev_max=lev_nbr_in-1;
	idx_lev_min=0L;
    ...
    if(flg_grd_out_hyb){
	 ...
	 Modify procedure for input coordinate to allow single-level output */
      ncr_idx_out=True;
      if(lev_nbr_out > 1){
	prs_max_out[0]=p0_out*hyam_out[0]+ps_out[0]*hybm_out[0];
	prs_max_out[1]=p0_out*hyam_out[1]+ps_out[0]*hybm_out[1];
	if(prs_max_out[1]-prs_max_out[0] < 0.0) ncr_idx_out=False;
      } /* !lev_nbr_out */
      if(ncr_idx_out){
	idx_lev_max=lev_nbr_out-1;
	idx_lev_min=0L;
  ...
  const int var_xcl_run_nbr_max=6; /* [nbr] Maximum number of variables in run-time exclusion list */
  ...
  if(flg_grd_out_dpt_3D){
	...
	(void)fprintf(stdout,"%s: DEBUG quark5 dmn_nbr_out = %d, dmn_nbr_dpt = %d, dmn_idx = %d, dmn_ids_out[%d] = %d, dmn_cnt_out[%d] = %ld, dmn_nm = %s\n",nco_prg_nm_get(),dmn_nbr_out,dmn_nbr_dpt,dmn_idx,dmn_idx,dmn_ids_out[dmn_idx],dmn_idx,dmn_cnt_out[dmn_idx],dmn_nm);
      } /* !dmn_idx */
    } /* !dbg */

    if(flg_mlc_out){
      int dmn_ids_hrz[2]; /* [id] Dimension IDs for horizontal variable */
      for(dmn_idx=0;dmn_idx<dmn_hrz_nbr_out;dmn_idx++)
	dmn_ids_hrz[dmn_idx]=dmn_ids_out[dmn_idx_hrz[dmn_idx]];
      rcd+=nco_def_var(out_id,mlc_nm,NC_INT,dmn_hrz_nbr_out,dmn_ids_hrz,&mlc_id);
      if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,mlc_id,NULL,nco_flt_flg_prc_fll);
      var_crt_nbr++;
    } /* !flg_mlc_out */
      
    if(flg_mlc_out){
      /* Define new metadata in interpolated file */
      nco_bool flg_need_msv=False; /* [flg] Add _FillValue attribute to variable */
      if(mlc_id_tpl != NC_MIN_INT) (void)nco_att_cpy(tpl_id,out_id,mlc_id_tpl,mlc_id,PCK_ATT_CPY);
      else if(mlc_id_in != NC_MIN_INT) (void)nco_att_cpy(fl_xtr_id,out_id,mlc_id_in,mlc_id,PCK_ATT_CPY);
      else rcd=nco_char_att_put(out_id,mlc_nm,"long_name","Index to the last active ocean cell in each column");
      for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++)
	if(mlc_out[grd_idx] == NC_FILL_INT)
	  break;
      if(grd_idx != grd_sz_out) flg_need_msv=True;
      has_mss_val=nco_mss_val_get_dbl(out_id,mlc_id,(double *)NULL);
  ...
  if(flg_grd_out_hyb){
    ...
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,hyam_id,NULL,nco_flt_flg_prc_fll);
    var_crt_nbr++;
    rcd+=nco_def_var(out_id,"hybi",crd_typ_out,dmn_nbr_1D,&dmn_id_ilev_out,&hybi_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,hybi_id,NULL,nco_flt_flg_prc_fll);
    var_crt_nbr++;
    rcd+=nco_def_var(out_id,"hybm",crd_typ_out,dmn_nbr_1D,&dmn_id_lev_out,&hybm_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,hybm_id,NULL,nco_flt_flg_prc_fll);
    var_crt_nbr++;
    rcd+=nco_def_var(out_id,ilev_nm_out,crd_typ_out,dmn_nbr_1D,&dmn_id_ilev_out,&ilev_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,ilev_id,NULL,nco_flt_flg_prc_fll);
    var_crt_nbr++;
  ...
  if(need_prs_ntf && (flg_grd_in_dpt_3D || flg_grd_out_dpt_3D)){
    /* Construct interface levels of depth/height grids from midpoint levels */
    if(flg_grd_in_dpt_3D){
      dpt_ntf_in=(double *)nco_malloc_dbg(tm_nbr_in*ilev_nbr_in*grd_sz_in*nco_typ_lng(var_typ_rgr),fnc_nm,"Unable to malloc() dpt_ntf_in value buffer");
      for(tm_idx=0;tm_idx<tm_nbr;tm_idx++){
    	/* Index-offset to current depth timeslice */
	idx_fst_lev=tm_idx*grd_sz_in*lev_nbr_in;
	idx_fst_ilev=tm_idx*grd_sz_in*ilev_nbr_in;	
	if(flg_hrz_mrv){
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++){
	    /* Assume depth/height = 0.0 in top/bottom interface */
	    dpt_ntf_in[idx_fst_ilev+grd_idx]=0.0;
	    /* Linearly extrapolate depth/height to bottom/top interface */
	    dpt_ntf_in[idx_fst_ilev+(ilev_nbr_in-1)*grd_sz_in+grd_idx]=dpt_mdp_in[idx_fst_lev+(lev_nbr_in-1)*grd_sz_in+grd_idx]-0.5*(dpt_mdp_in[idx_fst_lev+(lev_nbr_in-2)*grd_sz_in+grd_idx]-dpt_mdp_in[idx_fst_lev+(lev_nbr_in-1)*grd_sz_in+grd_idx]);
	    for(ilev_idx=1;ilev_idx<ilev_nbr_in-1;ilev_idx++)
	      dpt_ntf_in[idx_fst_ilev+ilev_idx*grd_sz_in+grd_idx]=0.5*(dpt_mdp_in[idx_fst_lev+(ilev_idx-1)*grd_sz_in+grd_idx]+dpt_mdp_in[idx_fst_lev+ilev_idx*grd_sz_in+grd_idx]);
	  } /* !grd_idx */
	}else{ /* !flg_hrz_mrv */
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++){
	    /* Assume depth/height = 0.0 in top/bottom interface */
	    dpt_ntf_in[idx_fst_ilev+grd_idx*ilev_nbr_in]=0.0;
	    /* Linearly extrapolate depth/height to bottom/top interface */
	    dpt_ntf_in[idx_fst_ilev+ilev_nbr_in-1+grd_idx*ilev_nbr_in]=dpt_mdp_in[idx_fst_lev+grd_idx*lev_nbr_in+lev_nbr_in-1]-0.5*(dpt_mdp_in[idx_fst_lev+grd_idx*lev_nbr_in+lev_nbr_in-2]-dpt_mdp_in[idx_fst_lev+grd_idx*lev_nbr_in+lev_nbr_in-1]);
	    for(ilev_idx=1;ilev_idx<ilev_nbr_in-1;ilev_idx++)
	      dpt_ntf_in[idx_fst_ilev+grd_idx*ilev_nbr_in+ilev_idx]=0.5*(dpt_mdp_in[idx_fst_lev+grd_idx*lev_nbr_in+ilev_idx-1]+dpt_mdp_in[idx_fst_lev+grd_idx*lev_nbr_in+ilev_idx]);
	  } /* !grd_idx */
	} /* !flg_hrz_mrv */
      } /* !tm_idx */
    } /* !flg_grd_in_dpt_3D */
    if(flg_grd_out_dpt_3D){
      dpt_ntf_out=(double *)nco_malloc_dbg(tm_nbr_out*ilev_nbr_out*grd_sz_out*nco_typ_lng(var_typ_rgr),fnc_nm,"Unable to malloc() dpt_ntf_out value buffer");
      for(tm_idx=0;tm_idx<tm_nbr;tm_idx++){
    	/* Index-offset to current depth timeslice */
	idx_fst_lev=tm_idx*grd_sz_out*lev_nbr_out;
	idx_fst_ilev=tm_idx*grd_sz_out*ilev_nbr_out;	
	if(flg_hrz_mrv){
	  for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++){
	    /* Assume depth/height = 0.0 in top/bottom interface */
	    dpt_ntf_out[idx_fst_ilev+grd_idx]=0.0;
	    /* Linearly extrapolate depth/height to bottom/top interface */
	    dpt_ntf_out[idx_fst_ilev+(ilev_nbr_out-1)*grd_sz_out+grd_idx]=dpt_mdp_out[idx_fst_lev+(lev_nbr_out-1)*grd_sz_out+grd_idx]-0.5*(dpt_mdp_out[idx_fst_lev+(lev_nbr_out-2)*grd_sz_out+grd_idx]-dpt_mdp_out[idx_fst_lev+(lev_nbr_out-1)*grd_sz_out+grd_idx]);
	    for(ilev_idx=1;ilev_idx<ilev_nbr_out-1;ilev_idx++)
	      dpt_ntf_out[idx_fst_ilev+ilev_idx*grd_sz_out+grd_idx]=0.5*(dpt_mdp_out[idx_fst_lev+(ilev_idx-1)*grd_sz_out+grd_idx]+dpt_mdp_out[idx_fst_lev+ilev_idx*grd_sz_out+grd_idx]);
	  } /* !grd_idx */
	}else{ /* !flg_hrz_mrv */
	  for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++){
	    /* Assume depth/height = 0.0 in top/bottom interface */
	    dpt_ntf_out[idx_fst_ilev+grd_idx*ilev_nbr_out]=0.0;
	    /* Linearly extrapolate depth/height to bottom/top interface */
	    dpt_ntf_out[idx_fst_ilev+ilev_nbr_out-1+grd_idx*ilev_nbr_out]=dpt_mdp_out[idx_fst_lev+grd_idx*lev_nbr_out+lev_nbr_out-1]-0.5*(dpt_mdp_out[idx_fst_lev+grd_idx*lev_nbr_out+lev_nbr_out-2]-dpt_mdp_out[idx_fst_lev+grd_idx*lev_nbr_out+lev_nbr_out-1]);
	    for(ilev_idx=1;ilev_idx<ilev_nbr_out-1;ilev_idx++)
	      dpt_ntf_out[idx_fst_ilev+grd_idx*ilev_nbr_out+ilev_idx]=0.5*(dpt_mdp_out[idx_fst_lev+grd_idx*lev_nbr_out+ilev_idx-1]+dpt_mdp_out[idx_fst_lev+grd_idx*lev_nbr_out+ilev_idx]);
	  } /* !grd_idx */
	} /* !flg_hrz_mrv */
      } /* !tm_idx */
    } /* !flg_grd_in_dpt_3D */
  ...
  if(flg_grd_out_dpt_3D){
    (void)nco_put_var(out_id,dpt_id,dpt_mdp_out,crd_typ_out);
  ...
  if(ntp_mth == nco_ntp_lnr || flg_grd_out_dpt) flg_ntp_log=False;
  size_t idx_fst_in; /* [idx] Index offset to input variables due to non-spatial (LRV) dimensions */
  size_t idx_fst_out; /* [idx] Index offset to output variables due to non-spatial (LRV) dimensions */
  size_t idx_in; /* [idx] Index into 3D input variables */
  size_t idx_out; /* [idx] Index into 3D output variables */
  size_t lrv_idx; /* [nbr] Index of non-spatial dimensions */
  size_t lrv_nbr; /* [nbr] Number of columns per gridcell per OpenMP loop */
  size_t mrv_nbr_in; /* [nbr] Number of spatial gridcells (horizontal times vertical) in input */
  size_t mrv_nbr_out; /* [nbr] Number of spatial gridcells (horizontal times vertical) in output */
  ...
  for(tm_idx=0;tm_idx<tm_nbr;tm_idx++){
    ...
    /* Index-offset to current depth/height timeslice */
    idx_fst_lev=tm_idx*grd_sz_in*lev_nbr_in;
    idx_fst_ilev=tm_idx*grd_sz_in*ilev_nbr_in;	
	...
	if(flg_grd_in_dpt_3D)
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++)
	    for(lev_idx=0;lev_idx<lev_nbr_in;lev_idx++)
	      prs_mdp_in[grd_idx+lev_idx*grd_sz_in]=dpt_mdp_in[idx_fst_lev+lev_idx*grd_sz_in+grd_idx];
	if(flg_grd_out_dpt_3D)
	  for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++)
	    for(lev_idx=0;lev_idx<lev_nbr_out;lev_idx++)
	      prs_mdp_out[grd_idx+lev_idx*grd_sz_out]=dpt_mdp_out[idx_fst_lev+lev_idx*grd_sz_out+grd_idx];
	if(flg_grd_in_hyb)
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++)
	    for(lev_idx=0;lev_idx<lev_nbr_in;lev_idx++)
	...
	if(flg_grd_in_prs || flg_grd_in_dpt_1D)
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++)
	    for(lev_idx=0;lev_idx<lev_nbr_in;lev_idx++)
	...
	if(flg_grd_in_dpt_3D)
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++)
	    for(lev_idx=0;lev_idx<lev_nbr_in;lev_idx++)
	      prs_mdp_in[lev_idx+grd_idx*lev_nbr_in]=dpt_mdp_in[idx_fst_lev+grd_idx*lev_nbr_in+lev_idx];
	if(flg_grd_out_dpt_3D)
	  for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++)
	    for(lev_idx=0;lev_idx<lev_nbr_out;lev_idx++)
	      prs_mdp_out[lev_idx+grd_idx*lev_nbr_out]=dpt_mdp_out[idx_fst_lev+grd_idx*lev_nbr_out+lev_idx];
	if(flg_grd_in_hyb)
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++)
	    for(lev_idx=0;lev_idx<lev_nbr_in;lev_idx++)
	...
	if(flg_grd_in_dpt_3D)
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++)
	    for(ilev_idx=0;ilev_idx<ilev_nbr_in;ilev_idx++)
	      prs_ntf_in[grd_idx+ilev_idx*grd_sz_in]=dpt_ntf_in[idx_fst_ilev+ilev_idx*grd_sz_in+grd_idx];
	if(flg_grd_out_dpt_3D)
	  for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++)
	    for(ilev_idx=0;ilev_idx<ilev_nbr_out;ilev_idx++)
	      prs_ntf_out[grd_idx+ilev_idx*grd_sz_out]=dpt_ntf_out[idx_fst_ilev+ilev_idx*grd_sz_out+grd_idx];
	if(flg_grd_in_hyb)
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++)
	    for(ilev_idx=0;ilev_idx<ilev_nbr_in;ilev_idx++)
	...
	if(flg_grd_in_dpt_3D)
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++)
	    for(ilev_idx=0;ilev_idx<ilev_nbr_in;ilev_idx++)
	      prs_ntf_in[ilev_idx+grd_idx*ilev_nbr_in]=dpt_ntf_in[idx_fst_ilev+grd_idx*ilev_nbr_in+ilev_idx];
	if(flg_grd_out_dpt_3D)
	  for(grd_idx=0;grd_idx<grd_sz_out;grd_idx++)
	    for(ilev_idx=0;ilev_idx<ilev_nbr_out;ilev_idx++)
	      prs_ntf_out[ilev_idx+grd_idx*ilev_nbr_out]=dpt_ntf_out[idx_fst_ilev+grd_idx*ilev_nbr_out+ilev_idx];
	if(flg_grd_in_hyb)
	  for(grd_idx=0;grd_idx<grd_sz_in;grd_idx++)
	    for(ilev_idx=0;ilev_idx<ilev_nbr_in;ilev_idx++)
...
# pragma omp parallel for firstprivate(has_ilev,has_lev,has_tm,var_val_dbl_in,var_val_dbl_out) private(dmn_cnt_in,dmn_cnt_out,dmn_id_in,dmn_id_out,dmn_idx,dmn_nbr_in,dmn_nbr_out,dmn_nbr_max,dmn_nm,dmn_srt,grd_idx,has_mss_val,idx_fst_in,idx_fst_out,idx_in,idx_out,idx_tbl,in_id,lrv_idx,lrv_nbr,lvl_idx_in,lvl_idx_out,lvl_nbr_in,lvl_nbr_out,mrv_nbr_in,mrv_nbr_out,mss_val_cmp_dbl,mss_val_dbl,ncr_idx_in,ncr_idx_out,prs_ntp_in,prs_ntp_out,rcd,thr_idx,trv,var_id_in,var_id_out,var_nm,var_sz_in,var_sz_out,var_typ_out,var_typ_rgr) shared(dmn_id_ilev_in,dmn_id_ilev_out,dmn_id_lev_in,dmn_id_lev_out,dmn_id_tm_in,flg_autoconvert,flg_hrz_mrv,flg_ntp_log,flg_vrt_tm,grd_nbr,idx_dbg,ilev_nbr_in,ilev_nbr_out,lev_nbr_in,lev_nbr_out,out_id,mlc_in,mlc_out,prs_mdp_in,prs_mdp_out,prs_ntf_in,prs_ntf_out,tm_idx,xtr_mth)
  ...
  (void)fprintf(stderr,"%s: INFO %s reports that GCC version constructed as integer is %d\n",nco_prg_nm_get(),fnc_nm,GCC_LIB_VERSION);
	...
	if(trv.flg_rgr){
	  ...
	  /* 20230405: How many vertical columns per gridcell in this OpenMP loop iteration?
	     Assume time, if any, is leading dimension and horizontal+vertical dimensions are last
	     Spatial dimensions are MRV and others (spectral, constituent, ensemble) are LRV */
	  lrv_nbr=var_sz_in/grd_nbr;
	  if(has_lev) lrv_nbr/=lev_nbr_in; else lrv_nbr/=ilev_nbr_in;
	  mrv_nbr_in=mrv_nbr_out=grd_nbr;
	  if(has_lev){
	    mrv_nbr_in*=lev_nbr_in;
	    mrv_nbr_out*=lev_nbr_out;
	  }else{
	  ...
	  if(flg_hrz_mrv){
	    if(prs_ntp_in[grd_nbr]-prs_ntp_in[0] > 0.0) ncr_idx_in=True; else ncr_idx_in=False;
	    ncr_idx_out=True;
	    if(out_nbr > 1)
	      if(prs_ntp_out[grd_nbr]-prs_ntp_out[0] < 0.0)
		ncr_idx_out=False;
	  }else{ /* !flg_hrz_mrv */
	    if(prs_ntp_in[1]-prs_ntp_in[0] > 0.0) ncr_idx_in=True; else ncr_idx_in=False;
	    ncr_idx_out=True;
	    if(out_nbr > 1)
	  ...
	  /* Outer loop over non-spatial dimensions */
	  for(lrv_idx=0;lrv_idx<lrv_nbr;lrv_idx++){
	    /* Loop over spatial columns */
	    for(grd_idx=0;grd_idx<grd_nbr;grd_idx++){
	      idx_fst_in=lrv_idx*mrv_nbr_in;
	      idx_fst_out=lrv_idx*mrv_nbr_out;
		...
		for(lvl_idx_in=0;lvl_idx_in<lvl_nbr_in;lvl_idx_in++){
		  ...
		  dat_in[lvl_idx_in]=var_val_dbl_in[idx_fst_in+idx_in];
		} /* !lvl_idx_in */
		...
		for(lvl_idx_out=0;lvl_idx_out<lvl_nbr_out;lvl_idx_out++){
		  idx_out=grd_idx+lvl_idx_out*grd_nbr;
		  var_val_dbl_out[idx_fst_out+idx_out]=dat_out[lvl_idx_out];
		} /* !lvl_idx_out */
	      }else{ /* !flg_hrz_mrv */
		for(lvl_idx_out=0;lvl_idx_out<lvl_nbr_out;lvl_idx_out++){
		  idx_out=grd_idx*lvl_nbr_out+lvl_idx_out;
...
#pragma omp critical
  ...
  if(dpt_mdp_out) dpt_mdp_out=(double *)nco_free(dpt_mdp_out);
  if(dpt_ntf_out) dpt_ntf_out=(double *)nco_free(dpt_ntf_out);
  if(mlc_out) mlc_out=(int *)nco_free(mlc_out);
  //if(bd_out) bd_out=(double *)nco_free(bd_out);
  //if(lt_out) lt_out=(double *)nco_free(lt_out);
 ...
 trv_tbl_sct * const trv_tbl) /* I/O [sct] Traversal Table */
{
  ...
  char *fl_in;
  char *fl_nlm=NULL; /* [sng] File containing nonlinear mapping weights from source to destination grid */
  char *fl_pth_lcl=NULL;
  ...
  if(1){
    ...
    for(idx=0;idx<sz;idx++){
      idx_row=row_dst_adr[idx]-1L; /* Convert from on-disk 1-based Fortran index to NCO 0-based C index */
    ...
    if(idx != sz){
      (void)fprintf(stderr,"%s: WARNING %s (aka \"the regridder\") reports map-file variable \"row\" contains %lu questionable value(s) among %lu total values. \"row\" contains indexes into the weight matrix S. First questionable value found is, in Fortran (1-based) index notation, row(%lu) = %ld. Each Fortran-convention index must be >= 1. Questionable indexes make this map-file suspect.\nHINT: Re-generate this map and, before using it, check it with \"ncks --chk_map map.nc\"\n",nco_prg_nm_get(),fnc_nm,cnt_bad,sz,(size_t)(idx+1L),idx_row+1L);
      //nco_exit(EXIT_FAILURE);
    } /* !idx */
    cnt_bad=0L;
    for(idx=0;idx<sz;idx++){
      idx_col=col_src_adr[idx]-1L; /* Convert from on-disk 1-based Fortran index to NCO 0-based C index */
    ...
    if(idx != sz){
      (void)fprintf(stderr,"%s: WARNING %s (aka \"the regridder\") reports map-file variable \"col\" contains %lu questionable value(s) among %lu total values. \"col\" contains indexes into the weight matrix S. First questionable value found is, in Fortran (1-based) index notation, col(%lu) = %ld. Each Fortran-convention index must be >= 1. Questionable indexes make this map-file suspect.\nHINT: Re-generate this map and, before using it, check it with \"ncks --chk_map map.nc\"\n",nco_prg_nm_get(),fnc_nm,cnt_bad,sz,(size_t)(idx+1L),idx_col+1L);
      //nco_exit(EXIT_FAILURE);
    } /* !idx */
  } /* !1 */
 ...
 skp_cf: 
  ...
  if(flg_grd_out_1D){
    ...
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,lat_bnd_id,NULL,nco_flt_flg_prc_fll);
    var_crt_nbr++;
    dmn_ids_out[0]=dmn_id_col;
    dmn_ids_out[1]=dmn_id_bnd;
    rcd+=nco_def_var(out_id,lon_bnd_nm_out,crd_typ_out,dmn_nbr_2D,dmn_ids_out,&lon_bnd_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,lon_bnd_id,NULL,nco_flt_flg_prc_fll);
    var_crt_nbr++;
    if(flg_area_out){
  ...
  if(flg_grd_out_rct){
    ...
    if(nco_grd_lat_typ == nco_grd_lat_fv && flg_stg){
      ...
      if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,slat_out_id,NULL,nco_flt_flg_prc_fll);
      var_crt_nbr++;
      rcd+=nco_def_var(out_id,slat_wgt_nm_out,crd_typ_out,dmn_nbr_1D,&dmn_id_slat,&slat_wgt_id);
      if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,slat_wgt_id,NULL,nco_flt_flg_prc_fll);
      var_crt_nbr++;
      rcd+=nco_def_var(out_id,slon_nm_out,crd_typ_out,dmn_nbr_1D,&dmn_id_slon,&slon_out_id);
      if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,slon_out_id,NULL,nco_flt_flg_prc_fll);
      var_crt_nbr++;
    } /* !nco_grd_lat_fv */
    dmn_ids_out[0]=dmn_id_lat;
    dmn_ids_out[1]=dmn_id_bnd;
    ...
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,lat_wgt_id,NULL,nco_flt_flg_prc_fll);
    var_crt_nbr++;
    dmn_ids_out[0]=dmn_id_lat;
    dmn_ids_out[1]=dmn_id_lon;
    if(flg_area_out){
  ...
  if(flg_add_fll){
	...
	if(lnk_idx == lnk_nbr){
	  ...
	  flg_dst_mpt=True;
	  /* Break dst_idx loop then print reassuring message that all empty cells will be _FillValue */
	  break;
	} /* !lnk_idx */
      } /* !dst_idx */
#else /* !0 */
  ...
  /* Needed for sgs_frc_* and caas_sgs_frc_* allocations */
  ...
  if(nlwgt_raw){
    ...
    if(sgs_frc_out){
      ...
      for(dst_idx=0;dst_idx<grd_sz_out;dst_idx++) caas_sgs_frc_out[dst_idx]=1.0;
      for(src_idx=0;src_idx<grd_sz_in;src_idx++) caas_sgs_frc_in[src_idx]=1.0;
    } /* !sgs_frc_out */
  }  /* !nlwgt_raw */
...
# pragma omp parallel for firstprivate(caas_tally,caas_wgt_vld_out,dmn_cnt_in,dmn_cnt_out,dmn_srt,dmn_id_in,dmn_id_out,tally,var_val_dbl_in,var_val_dbl_out,wgt_vld_out) private(dmn_idx,dmn_nbr_in,dmn_nbr_out,dmn_nbr_max,dst_idx,has_mss_val,idx,idx_in,idx_out,idx_tbl,in_id,lnk_idx,lvl_idx,lvl_nbr,mss_val_cmp_dbl,mss_val_dbl,nllnk_idx,rcd,thr_idx,trv,val_in_fst,val_out_fst,var_id_in,var_id_out,var_nm,var_sz_in,var_sz_out,var_typ_out,var_typ_rgr,var_val_crr) shared(caas_sgs_frc_in,caas_sgs_frc_out,col_src_adr,dmn_nbr_hrz_crd,flg_add_fll,flg_autoconvert,flg_frc_nrm,flg_mpt_mss,flg_msk_apl,flg_msk_out,frc_out,lnk_nbr,msk_out,nlcol_src_adr,nllnk_nbr,nlrow_dst_adr,nlwgt_raw,out_id,row_dst_adr,sgs_frc_nm,sgs_frc_in,sgs_frc_out,sgs_msk_nm,wgt_raw)
  ...
  for(idx_tbl=0;idx_tbl<trv_nbr;idx_tbl++){
	...
	/* Determining whether an individual field _uses_ missing values is important because
	   ...
	   Now has_mss_val means: does the variable use the explicitly defined missing value?
	   Or, failing that, does the variable use the implicitly defined missing value?
...
# endif /* !__GNUC__ */
	...
	if(sgs_frc_out){
	  if(!strcmp(var_nm,sgs_frc_nm)){
	    ...
	    /* Set output mask to true based on shared sgs_frc_out (which is guaranteed to be all valid values) */
	    for(dst_idx=0;dst_idx<grd_sz_out;dst_idx++)
	...
	if(flg_add_fll && !has_mss_val){
	  /* 20210604: Initialize fields that do not use _FillValue in input file to default missing value in unmapped destination cells
	  ...
	  /* 20221018: Above clause successfully sets continental gridpoints to _FillValue in MPAS output
	     However, it leaves cells with no local SGS contribution equal to zero instead of _FillValue
	     In MPAS-Seaice output, for example, every ocean cell without sea ice is still 0.0 not _FillValue
	     MPAS-Seaice datasets may contain the floating-point (not integer) ice-presence mask timeMonthly_avg_icePresentIntMask
	     --msk_apl should help set to _FillValue any gridcells with timeMonthly_avg_icePresentIntMask == 0.0
	     However, input SGS datasets that lack a mask variable cannot use --msk_apl to mask empty SGS cells
	     Moreover, regridder still treats mask_b in map-file as msk_out, and never reads-in sgs_msk_nm! Doh!
	     Clearly, if regridder is called with sgs_msk_nm then regridder should set msk_out == sgs_msk for all variables
	     This is a high-priority feature for a future release (meaning 202311?)
	  ...
	  if(sgs_frc_out){
	    ...
	    if(flg_mpt_mss){ 
	      for(dst_idx=0;dst_idx<grd_sz_out;dst_idx++){
		/* 20221018: Mask where sgs_frc_out is empty */
	...
	if(nlwgt_raw){
	  double *caas_gym=NULL;
	  caas_gym=(double *)nco_malloc_dbg(var_sz_out*nco_typ_lng(var_typ_rgr),fnc_nm,"Unable to malloc() caas_gym output value buffer");
	  /* Initialize output */
	  ...
	  sub_sgs_frc_out=(double *)nco_malloc_dbg(grd_sz_out*nco_typ_lng(var_typ_rgr),fnc_nm,"Unable to malloc() sub_sgs_frc_out value buffer");
	  memcpy(sub_sgs_frc_out,caas_sgs_frc_out,grd_sz_out*nco_typ_lng(var_typ_rgr));

	  /* CAAS-SGS-regrid single-level fields with missing values */
	  for(lnk_idx=0;lnk_idx<lnk_nbr;lnk_idx++){
	    idx_in=col_src_adr[lnk_idx];
	    idx_out=row_dst_adr[lnk_idx];
...
{
  /* Purpose: Convert vertical grid-type enum to string */
  switch(nco_vrt_grd_typ){
  case nco_vrt_grd_nil: return "Unknown, unclassified, or unrepresentable vertical grid type";
  case nco_vrt_grd_hyb: return "Hybrid sigma-pressure grid, used by CCM 1-3, CAM 1-6, ECMWF IFS";
  case nco_vrt_grd_prs: return "Pure pressure coordinate grid, used by NCEP";
...
(rgr_sct * const rgr) /* I/O [sct] Regridding structure */
{
  ...
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,area_id,NULL,nco_flt_flg_prc_fll);
  (void)nco_def_var(out_id,msk_nm,(nc_type)NC_INT,dmn_nbr_1D,&dmn_id_grd_sz,&msk_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,msk_id,NULL,nco_flt_flg_prc_fll);
  ...
  if(fl_out){
    ...
    if(flg_grd_1D){
      ...
      if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,lat_id,NULL,nco_flt_flg_prc_fll);
      (void)nco_def_var(out_id,lon_nm_out,crd_typ,dmn_nbr_1D,&dmn_id_col,&lon_id);
      if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,lon_id,NULL,nco_flt_flg_prc_fll);
...
(rgr_sct * const rgr) /* I/O [sct] Regridding structure */
{
 ...
 skp_cf: 
  ...
  if(fl_out){
    ...
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,dg_nd_id,NULL,nco_flt_flg_prc_fll);
    dmn_ids[0]=dmn_id_fc;
    dmn_ids[1]=dmn_id_npf;
    rcd=nco_def_var(out_id,fc_nd_nm,(nc_type)NC_INT,dmn_nbr_2D,dmn_ids,&fc_nd_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,fc_nd_id,NULL,nco_flt_flg_prc_fll);
    rcd=nco_def_var(out_id,msh_nm,(nc_type)NC_INT,dmn_nbr_0D,(int *)NULL,&msh_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,msh_id,NULL,nco_flt_flg_prc_fll);
    rcd=nco_def_var(out_id,ndx_nm,crd_typ,dmn_nbr_1D,&dmn_id_nd,&ndx_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,ndx_id,NULL,nco_flt_flg_prc_fll);
    rcd=nco_def_var(out_id,ndy_nm,crd_typ,dmn_nbr_1D,&dmn_id_nd,&ndy_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,ndy_id,NULL,nco_flt_flg_prc_fll);
    rcd=nco_def_var(out_id,dgx_nm,crd_typ,dmn_nbr_1D,&dmn_id_dg,&dgx_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,dgx_id,NULL,nco_flt_flg_prc_fll);
    rcd=nco_def_var(out_id,dgy_nm,crd_typ,dmn_nbr_1D,&dmn_id_dg,&dgy_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,dgy_id,NULL,nco_flt_flg_prc_fll);
    rcd=nco_def_var(out_id,fcx_nm,crd_typ,dmn_nbr_1D,&dmn_id_fc,&fcx_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,fcx_id,NULL,nco_flt_flg_prc_fll);
    rcd=nco_def_var(out_id,fcy_nm,crd_typ,dmn_nbr_1D,&dmn_id_fc,&fcy_id);
    if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,fcy_id,NULL,nco_flt_flg_prc_fll);
