 ...
 skp_cf: 
  ...
  if(!lat_dmn_nm || !lon_dmn_nm){
    if(!lat_dmn_nm){
      ...
      else if((rcd=nco_inq_dimid_flg(in_id,"y",&dmn_id_lat)) == NC_NOERR) lat_dmn_nm=strdup("y"); /* NSIDC polar stereographic */
    ...
    if(!lon_dmn_nm){
      ...
      else if((rcd=nco_inq_dimid_flg(in_id,"x",&dmn_id_lon)) == NC_NOERR) lon_dmn_nm=strdup("x"); /* NSIDC polar stereographic */
