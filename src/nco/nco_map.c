...
(rgr_sct * const rgr) /* I/O [sct] Regridding structure */
{
  ...
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,area_in_id,NULL,nco_flt_flg_prc_fll);
  rcd+=nco_def_var(out_id,"frac_a",crd_typ,dmn_nbr_1D,&src_grid_size_id,&frc_in_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,frc_in_id,NULL,nco_flt_flg_prc_fll);
  rcd+=nco_def_var(out_id,"mask_a",(nc_type)NC_INT,dmn_nbr_1D,&src_grid_size_id,&msk_in_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,msk_in_id,NULL,nco_flt_flg_prc_fll);
  rcd+=nco_def_var(out_id,"xc_a",crd_typ,dmn_nbr_1D,&src_grid_size_id,&src_grd_ctr_lon_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,src_grd_ctr_lon_id,NULL,nco_flt_flg_prc_fll);
  rcd+=nco_def_var(out_id,"yc_a",crd_typ,dmn_nbr_1D,&src_grid_size_id,&src_grd_ctr_lat_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,src_grd_ctr_lat_id,NULL,nco_flt_flg_prc_fll);
  ...
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,src_grd_crn_lon_id,NULL,nco_flt_flg_prc_fll);
  dmn_ids[0]=src_grid_size_id;
  dmn_ids[1]=src_grid_corners_id;
  rcd+=nco_def_var(out_id,"yv_a",crd_typ,dmn_nbr_2D,dmn_ids,&src_grd_crn_lat_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,src_grd_crn_lat_id,NULL,nco_flt_flg_prc_fll);
  ...
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,frc_out_id,NULL,nco_flt_flg_prc_fll);
  rcd+=nco_def_var(out_id,"mask_b",(nc_type)NC_INT,dmn_nbr_1D,&dst_grid_size_id,&msk_out_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,msk_out_id,NULL,nco_flt_flg_prc_fll);
  rcd+=nco_def_var(out_id,"xc_b",crd_typ,dmn_nbr_1D,&dst_grid_size_id,&dst_grd_ctr_lon_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,dst_grd_ctr_lon_id,NULL,nco_flt_flg_prc_fll);
  rcd+=nco_def_var(out_id,"yc_b",crd_typ,dmn_nbr_1D,&dst_grid_size_id,&dst_grd_ctr_lat_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,dst_grd_ctr_lat_id,NULL,nco_flt_flg_prc_fll);
  ...
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,dst_grd_crn_lon_id,NULL,nco_flt_flg_prc_fll);
  dmn_ids[0]=dst_grid_size_id;
  dmn_ids[1]=dst_grid_corners_id;
  rcd+=nco_def_var(out_id,"yv_b",crd_typ,dmn_nbr_2D,dmn_ids,&dst_grd_crn_lat_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,dst_grd_crn_lat_id,NULL,nco_flt_flg_prc_fll);
  ...
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,col_src_adr_id,NULL,nco_flt_flg_prc_fll);
  rcd+=nco_def_var(out_id,"row",(nc_type)NC_INT,dmn_nbr_1D,&num_links_id,&row_dst_adr_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,row_dst_adr_id,NULL,nco_flt_flg_prc_fll);
  rcd+=nco_def_var(out_id,"S",crd_typ,dmn_nbr_1D,&num_links_id,&wgt_raw_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,wgt_raw_id,NULL,nco_flt_flg_prc_fll);
 ...
 size_t *lnk_nbr_ptr) /* O [nbr] Number of links */
{
  ...
  //  test nco_poly functions
  {
    ...
    /* DEBUG output all polygons
    for(idx=0;idx<pl_cnt_out;idx++) nco_poly_prn(pl_lst_out[idx],1);
    */
  }
  ...
  if(rgr->wgt_typ == nco_wgt_con){
    ...
    if(nco_dbg_lvl_get() >= nco_dbg_dev)
      (void)fprintf(stderr,"%s: INFO: num input polygons=%lu, num output polygons=%lu num overlap polygons=%d\n",nco_prg_nm_get(),grd_sz_in,grd_sz_out,pl_cnt_vrl);
 ...
 double *lon_crn)
{
  ...
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,grd_crn_lat_id,NULL,nco_flt_flg_prc_fll);

  (void)nco_def_var(out_id,grd_crn_lon_nm,crd_typ,dmn_nbr_2D,dmn_ids,&grd_crn_lon_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,grd_crn_lon_id,NULL,nco_flt_flg_prc_fll);

  (void)nco_def_var(out_id,grd_area_nm,crd_typ,dmn_nbr_1D,dmn_ids,&grd_area_id);
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,grd_area_id,NULL,nco_flt_flg_prc_fll);
...
int fl_out_fmt)
{
  ...
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,grd_ctr_lon_id,NULL,nco_flt_flg_prc_fll);
  ...
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,grd_ctr_lat_id,NULL,nco_flt_flg_prc_fll);
  ...
  if(nco_cmp_glb_get()) rcd+=nco_flt_def_out(out_id,grd_msk_id,NULL,nco_flt_flg_prc_fll);
