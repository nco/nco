/* $Header: /data/zender/nco_20150216/nco/src/nco/nco_var_lst.c,v 1.49 2005-08-03 18:08:04 zender Exp $ */
 ...
 int * const nbr_xtr) /* I/O [nbr] Number of variables in current extraction list */
{
  ...
  for(idx=0;idx<*nbr_xtr;idx++){
    ...
    if(strpbrk(var_sng,".*^$\\[]()<>+?|{}")){
      ...
      continue;
#else
      (void)fprintf(stdout,"%s: ERROR: Sorry, wildcarding (extended regular expression matches to variables) is unavailable, so unable to compile regular expression \"%s\".\nHINT: Make sure libregex.a is on path and re-build NCO.\n",prg_nm_get(),var_sng);
 ...
 int * const nbr_var_prc) /* O [nbr] Number of processed variables */
{
  ...
  for(idx=0;idx<nbr_var;idx++){
    ...
    if(NCAR_CCSM_FORMAT){
      ...
      if(prg == ncbo && (!strcmp(var_nm,"hyam") || !strcmp(var_nm,"hybm") || !strcmp(var_nm,"hyai") || !strcmp(var_nm,"hybi") || !strcmp(var_nm,"gw") || !strcmp(var_nm,"lon_bnds") || !strcmp(var_nm,"lat_bnds") || !strcmp(var_nm,"area") || !strcmp(var_nm,"ORO") || !strcmp(var_nm,"date") || !strcmp(var_nm,"datesec")) || strstr(var_nm,"msk_")) var_op_typ[idx]=fix;
