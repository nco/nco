/* $Header: /data/zender/nco_20150216/nco/src/nco/nco_fl_utl.c,v 1.106 2008-09-02 17:25:50 zender Exp $ */
...
/* #endif HAVE_RES_ */
    ...
    if(rmt_cmd == NULL){
      (void)fprintf(stderr,"%s: ERROR %s neither exists locally nor agrees with remote filename patterns\n",prg_nm_get(),fl_nm_rmt);
      nco_exit(EXIT_FAILURE);
    } /* end if */
...
#define USR_RPL_MAX_NBR 10 /* [nbr] Maximum number of chances for user to reply */
	...
	nco_exit(EXIT_FAILURE);
      ...
      (void)fprintf(stdout,"%s: %s exists---`e'xit, `o'verwrite (i.e., delete existing file), or `a'ppend (i.e., replace duplicate and add new variables to existing file) (e/o/a)? ",prg_nm_get(),fl_out);
      (void)fflush(stdout);
