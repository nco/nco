/* $Header: /data/zender/nco_20150216/nco/src/nco/nco_cnv_csm.c,v 1.42 2009-04-19 23:17:04 zender Exp $ */
...
(const int nc_id) /* I [id] netCDF file ID */
{
  ...
  if(rcd == NC_NOERR && att_typ == NC_CHAR){
    ...
    if(CNV_CCM_CCSM_CF && dbg_lvl_get() > 0){
      ...
      if(dbg_lvl_get() > 1) (void)fprintf(stderr,"%s: INFO NCO has a unified (though incomplete) treatment of many related (official and unoffical) conventions such as CCM, CCSM, and CF. As part of adhering to this convention, NCO implements variable-specific exceptions in certain operators, e.g., ncbo will not subtract variables named \"date\" or \"gw\". For a full list of exceptions, see the manual http://nco.sf.net/nco.html#CF\n",prg_nm_get());
    } /* endif dbg */
