/* $Header: /data/zender/nco_20150216/nco/src/nco/ncrename.c,v 1.171 2013-10-14 23:22:18 pvicente Exp $ */
...
main(int argc,char **argv)
{
  ...
  const char * const CVS_Id="$Id: ncrename.c,v 1.171 2013-10-14 23:22:18 pvicente Exp $"; 
    ...
    if(var_rnm_lst[idx_var].old_nm[0] == opt_chr){
      ...
      for(unsigned int idx_tbl=0;idx_tbl<trv_tbl->nbr;idx_tbl++){
        ...
        if (trv_tbl->lst[idx_tbl].nco_typ == nco_obj_typ_var){
          ...
          rcd=nco_inq_varid_flg(grp_id,var_rnm_lst[idx_var].old_nm+1L,&var_rnm_lst[idx_var].id);
          if(rcd == NC_NOERR){
            (void)nco_rename_var(grp_id,var_rnm_lst[idx_var].id,var_rnm_lst[idx_var].new_nm);
            if(dbg_lvl >= nco_dbg_std) (void)fprintf(stdout,"%s: Renamed variable \'%s\' to \'%s\'\n",prg_nm,var_rnm_lst[idx_var].old_nm+1L,var_rnm_lst[idx_var].new_nm);
          }else{
      ...
      for(unsigned int idx_tbl=0;idx_tbl<trv_tbl->nbr;idx_tbl++){
        ...
        if (trv_tbl->lst[idx_tbl].nco_typ == nco_obj_typ_var && strcmp(trv_tbl->lst[idx_tbl].nm,var_rnm_lst[idx_var].old_nm) == 0){
          (void)nco_inq_grp_full_ncid(nc_id,trv_tbl->lst[idx_tbl].grp_nm_fll,&grp_id);
          rcd=nco_inq_varid(grp_id,var_rnm_lst[idx_var].old_nm,&var_rnm_lst[idx_var].id);
          (void)nco_rename_var(grp_id,var_rnm_lst[idx_var].id,var_rnm_lst[idx_var].new_nm);
  ...
  for(int idx_grp=0;idx_grp<nbr_grp_rnm;idx_grp++){
    if(grp_rnm_lst[idx_grp].old_nm[0] == opt_chr){
      ...
      for(unsigned int idx_tbl=0;idx_tbl<trv_tbl->nbr;idx_tbl++){
        ...
        if (trv_tbl->lst[idx_tbl].nco_typ == nco_obj_typ_var){
          (void)nco_inq_grp_full_ncid(nc_id,trv_tbl->lst[idx_tbl].grp_nm_fll,&grp_id);
          rcd=nco_inq_grp_ncid_flg(grp_id,grp_rnm_lst[idx_grp].old_nm+1L,&grp_rnm_lst[idx_grp].id);
          if(rcd == NC_NOERR){
            (void)nco_rename_grp(grp_rnm_lst[idx_grp].id,grp_rnm_lst[idx_grp].new_nm);
            if(dbg_lvl >= nco_dbg_std) (void)fprintf(stdout,"%s: Renamed group \'%s\' to \'%s\'\n",prg_nm,grp_rnm_lst[idx_grp].old_nm+1L,grp_rnm_lst[idx_grp].new_nm);
          }else{
            (void)fprintf(stdout,"%s: WARNING Group \"%s\" not present in %s, skipping it.\n",prg_nm,grp_rnm_lst[idx_grp].old_nm+1L,fl_in);
            /* Reset error code */
            rcd=NC_NOERR; 
          }/* Loop table */
        } /* Is group */
      ...
      for(unsigned int idx_tbl=0;idx_tbl<trv_tbl->nbr;idx_tbl++){
        /* Match group by name */
        if (trv_tbl->lst[idx_tbl].nco_typ == nco_obj_typ_grp && strcmp(trv_tbl->lst[idx_tbl].nm,var_rnm_lst[idx_grp].old_nm) == 0){
          (void)nco_inq_grp_full_ncid(nc_id,trv_tbl->lst[idx_tbl].grp_nm_fll,&grp_id);
          rcd=nco_inq_grp_ncid(grp_id,grp_rnm_lst[idx_grp].old_nm,&grp_rnm_lst[idx_grp].id);
          (void)nco_rename_grp(grp_rnm_lst[idx_grp].id,grp_rnm_lst[idx_grp].new_nm);
  ...
  for(int idx_dmn=0;idx_dmn<nbr_dmn_rnm;idx_dmn++){
    if(dmn_rnm_lst[idx_dmn].old_nm[0] == opt_chr){
      ...
      for(unsigned int idx_tbl=0;idx_tbl<trv_tbl->nbr_dmn;idx_tbl++){
        ...
        rcd=nco_inq_dimid_flg(grp_id,dmn_rnm_lst[idx_dmn].old_nm+1L,&dmn_rnm_lst[idx_dmn].id);
        if(rcd == NC_NOERR){
  ...
  if(nbr_att_rnm > 0){
    ...
    /* Get number of variables in file */
    (void)trv_tbl_inq(NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,&nbr_var_fl,trv_tbl);
    ...
    for(int idx_att=0;idx_att<nbr_att_rnm;idx_att++){
      ...
      if(strchr(att_rnm_lst[idx_att].old_nm,dlm_chr)){
        ...
        for(unsigned int idx_tbl=0;idx_tbl<trv_tbl->nbr;idx_tbl++){
          /* Match variable by name */
          if (trv_tbl->lst[idx_tbl].nco_typ == nco_obj_typ_var && strcmp(trv_tbl->lst[idx_tbl].nm,var_nm) == 0){
            (void)nco_inq_grp_full_ncid(nc_id,trv_tbl->lst[idx_tbl].grp_nm_fll,&grp_id);
            ...
            if(IS_GLB_GRP_ATT){
              ...
              if(var_nm[0] == opt_chr){
                ...
                rcd=nco_inq_varid(grp_id,var_nm,&var_id);
              } /* end if */
            } /* !IS_GLB_GRP_ATT */
            if(rcd == NC_NOERR){
              if(att_rnm_lst[idx_att].old_nm[0] == opt_chr){
                ...
                if(rcd == NC_NOERR){
                  ...
                  nbr_rnm++;
                  if(dbg_lvl >= nco_dbg_std) (void)fprintf(stdout,"%s: Renamed attribute \'%s\' to \'%s\' for variable \'%s\'\n",prg_nm,att_rnm_lst[idx_att].old_nm+1L,att_rnm_lst[idx_att].new_nm,(var_nm[0] == opt_chr ? var_nm+1L : var_nm));
                }else{
                  (void)fprintf(stdout,"%s: WARNING Attribute \'%s\' not present in variable \'%s\'\n",prg_nm,att_rnm_lst[idx_att].old_nm+1L,(var_nm[0] == opt_chr ? var_nm+1L : var_nm));
                } /* endelse */
              }else{ 
                /* Attribute must be present */
                ...
                if(rcd == NC_NOERR){
                  ...
                  nbr_rnm++;
                  if(dbg_lvl >= nco_dbg_std) (void)fprintf(stdout,"%s: Renamed attribute \'%s\' to \'%s\' for variable \'%s\'\n",prg_nm,att_rnm_lst[idx_att].old_nm,att_rnm_lst[idx_att].new_nm,(var_nm[0] == opt_chr ? var_nm+1L : var_nm));
                } /* endif attribute is present */
              } /* endelse attribute must be present */  
            }else{ /* variable not present */
            ...
            if(att_rnm_lst[idx_att].old_nm[0] == opt_chr){
              ...
              if(rcd == NC_NOERR){
                ...
                if(var_id > NC_GLOBAL){
                  ...
                  if(dbg_lvl >= nco_dbg_std) (void)fprintf(stdout,"%s: Renamed global or group attribute \'%s\' to \'%s\'\n",prg_nm,att_rnm_lst[idx_att].old_nm+1L,att_rnm_lst[idx_att].new_nm);
                } /* end else */
              }else{ /* end if attribute was found */
                /* Reset error code */
                rcd=NC_NOERR; 
