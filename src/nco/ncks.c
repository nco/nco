...
main(int argc,char **argv)
{
  ...
  nco_bool flg_map_fix=False; /* [flg] Fix map-file weights */
  ...
  static struct option opt_lng[]={ /* Structure ordered by short option key if possible */
    ...
    {"map_fix",no_argument,0,0}, /* [flg] Fix map-file weights */
  ...
  while(1){
	...
	md5->wrt=md5->dgs=True;
      ...
      if(!strcmp(opt_crr,"map_fix") || !strcmp(opt_crr,"fix_map")) flg_map_fix=True;
...
#     sudo /bin/cp ~/nco/po/${LL}/nco.mo /usr/share/locale/${LL}/LC_MESSAGES  
  ...
  if(fl_out){
    ...
    if(flg_rgr){
      ...
      rgr_nfo=nco_rgr_ini(cmd_ln,in_id,rgr_arg,rgr_nbr,rgr_in,rgr_out,rgr_grd_src,rgr_grd_dst,rgr_map,rgr_var,rgr_vrt,wgt_vld_thr,xtn_lst_in,xtn_nbr);
	...
	nco_map_chk(fl_in,flg_area_wgt,flg_map_fix);
        goto close_and_free;
      } /* !CHK_MAP */
