/* $Header: /data/zender/nco_20150216/nco/src/nco/ncpdq.c,v 1.264 2013-06-12 20:23:55 pvicente Exp $ */
...
main(int argc,char **argv)
{
  ...
  const char * const CVS_Id="$Id: ncpdq.c,v 1.264 2013-06-12 20:23:55 pvicente Exp $"; 
  ...
  */
  ...
  if(IS_REORDER){
    ...
    for(int idx_var=0;idx_var<nbr_var_prc;idx_var++){
      ...
      /* If re-ordering, determine and set new dimensionality in metadata of each re-ordered variable */
      (void)nco_var_dmn_rdr_mtd_trv(var[idx_var],var_out[idx_var],dmn_idx_out_in[idx_var],dmn_rvr_rdr,dmn_rvr_in[idx_var],trv_tbl);
