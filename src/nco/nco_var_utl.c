/* $Header: /data/zender/nco_20150216/nco/src/nco/nco_var_utl.c,v 1.37 2004-07-06 05:40:42 zender Exp $ */
 ...
 var_sct *var) /* I [sct] Variable to get */
{
  ...
  if(dbg_lvl_get() > 2) (void)fprintf(stdout,"%s: DEBUG: fxm TODO nco334. Calling nco_get_vara() for variable %s with nc_id=%d, var_id=%d, var_srt=%d, var_cnt = %d, var_val = %d, var_typ = %s\n",prg_nm_get(),var->nm,nc_id,var->id,var->srt[0],var->cnt[0],var->val.dp[0],nco_typ_sng(var->typ_dsk));
#ifdef _OPENMP
#pragma omp critical
#endif /* _OPENMP */
