 ...
 const trv_tbl_sct * const trv_tbl) /* I [sct] GTT (Group Traversal Table) */
{
  ...
  for(rec_idx=0;rec_idx<rec_sz;rec_idx++){
	...
	if(flg_ppc){
 	  if(flg_nsd) (void)nco_ppc_bitmask(var_lst[var_idx]->grp_id_out,var_out_id,ppc,var_out.type,var_out.sz,var_out.has_mss_val,var_out.mss_val,var_out.val); else (void)nco_ppc_around(ppc,var_out.type,var_out.sz,var_out.has_mss_val,var_out.mss_val,var_out.val);
	} /* !PPC */
 ...
 int *cf_var_id) /* O [id] CF Variable ID */
{
  /* Purpose: Is variable specified in an associated attribute?
     Associated attributes include "ancillary_variables", "bounds", "climatology", "coordinates", "grid_mapping", "quantization"
  ...
  for(idx_var=0;idx_var<nbr_var;idx_var++){
	...
	if(att_typ == NC_CHAR){
	  ...
	  if(att_sz != 1L){
	    (void)fprintf(stderr,"%s: WARNING %s reports \"%s\" attribute for variable %s is an %s array of size %ld. This violates the CF Conventions which requires a single string for this attribute. NCO will skip this attribute.\n",nco_prg_nm_get(),fnc_nm,att_nm,var_nm,nco_typ_sng(att_typ),att_sz);
	    return IS_SPC_IN_CF_ATT;
	  } /* !att_sz */
 ...
 const int nbr_dim) /* I [nbr] Number of dimensions in list */
{
  ...
  /* 20130112: Variables associated with "bounds", "climatology", "coordinates", "grid_mapping", and "quantization" attributes should,
  ...
  if(nco_is_spc_in_cf_att(var->nc_id,"quantization",var->id,NULL)) var->is_crd_var=True;
