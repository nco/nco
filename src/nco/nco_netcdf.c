/* $Header: /data/zender/nco_20150216/nco/src/nco/nco_netcdf.c,v 1.195 2013-03-29 05:39:40 zender Exp $ */
 ...
 const char * const msg) /* I [sng] Supplemental error message */
{
  ...
  case NC_ENOTNC: (void)fprintf(stdout,"ERROR NC_ENOTNC Not a netCDF file\nHINT: NC_ENOTNC errors can occur for many reasons; decide if any of these apply to your use case and take the appropriate action:\n1. Often NC_ENOTNC occurs when NCO operators linked to the netCDF3 library attempt to read netCDF4 files. ");
    ...
    (void)fprintf(stdout,"2. NC_ENOTNC can occur when users attempt to utilize diskless (i.e., RAM) files.  In this case remove the diskless switches (e.g., --ram or --diskless) and then re-issue the command.\n3. NC_ENOTNC can also occur when users attempt to read other non-supported filetypes, e.g., HDF4, HDF-EOS2. In this case it may be possible to access the input files using NCO if NCO is first re-linked to a version of netCDF configured with the --enable-hdf4 option. This is a non-standard netCDF build option.\n4. NC_ENOTNC can occur when access to a DAP URL fails, and the backup method of downloading the URL using wget obtains a data aggregation file (e.g., a .ncml file) instead of an actual netCDF file. In this case the problem is with the DAP server or URL.\n"); break;
...
int nco_inq_grp_parent_flg(const int nc_id,int * const prn_id){assert(0);return NC_NOERR;}
