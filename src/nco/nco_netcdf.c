 ...
 const size_t * const cnk_sz) /* [nbr] Chunk sizes */
{
  ...
  if(rcd == NC_EBADCHUNK){
    ...
    if(sz > NCO_MAX_CHUNK_SIZE) (void)fprintf(stderr, "%s: ERROR Total requested chunk size = %lu exceeds netCDF maximium-supported chunk size = %ld\n",fnc_nm,sz,NCO_MAX_CHUNK_SIZE);
  } /* !rcd */
