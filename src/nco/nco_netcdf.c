...
(const int fl_fmt) /* I [enm] netCDF file format */
{
  ...
  switch(fl_fmt){
  case NC_FORMAT_CLASSIC:
    return "classic = NC_FORMAT_CLASSIC = CDF1. This is the first generation (and least capacious) format of Unidata's Classic Data Model: https://docs.unidata.ucar.edu/netcdf-c/current/netcdf_data_model.html. Chunking and compression are not supported. This format limits fixed variables to sizes smaller than 2^31 B = 2 GiB ~ 2.1 GB, and record variables to that size per record. A single variable may exceed this limit if and only if it is the last defined variable.";
  case NC_FORMAT_64BIT_OFFSET:
    return "64-bit offset = NC_FORMAT_64BIT_OFFSET = CDF2. This is the second generation (and more capacious) format of Unidata's Classic Data Model: https://docs.unidata.ucar.edu/netcdf-c/current/netcdf_data_model.html. Chunking and compression are not supported. This format limits fixed variables to sizes smaller than 2^32 B = 4 GiB ~ 4.2 GB, and record variables to that size per record. Any number of variables may reach, though not exceed, this size for fixed variables, or this size per record for record variables.";
  case NC_FORMAT_NETCDF4:
    return "netCDF-4 = NC_FORMAT_NETCDF4. This format embodies all the features of Unidata's Enhanced Data Model: https://docs.unidata.ucar.edu/netcdf-c/current/netcdf_data_model.html. Chunking and compression are supported. This format has no variable size limitations of real-world import.";
  case NC_FORMAT_NETCDF4_CLASSIC:
    return "netCDF-4 classic model = NC_FORMAT_NETCDF4_CLASSIC. This is a hybrid of Unidata's Enhanced Data Model that implements the netCDF4 storage format yet only allows data types from the Unidata's Classic Model: https://docs.unidata.ucar.edu/netcdf-c/current/netcdf_data_model.html. Chunking and compression are supported. This format has no variable-size limitations of real-world import.";
  case NC_FORMAT_CDF5:
    return "cdf5 = NC_FORMAT_CDF5 = CDF5 = PnetCDF = PIO. This is third generation (and most modern and capacious) format of Unidata's Classic Data Model, https://docs.unidata.ucar.edu/netcdf-c/current/netcdf_data_model.html. Chunking and compression are not supported. This format has no variable-size limitations of real-world import.";
  default: nco_dfl_case_nc_type_err(); break;
  } /* !fl_fmt */
...
nco_put_vara(const int nc_id,const int var_id,const long * const srt,const long * const cnt,const void * const vp,const nc_type var_typ)
{
  ...
  if(rcd != NC_NOERR){
    ...
    if(rcd == NC_EEDGE){
      ...
      (void)fprintf(stdout,"Start and Count Vectors NCO passed to %s to write:\ndmn_idx\tsrt\tcnt\n",var_nm);
      for(int dmn_idx=0;dmn_idx<dmn_nbr;dmn_idx++)
	(void)fprintf(stdout,"%d\t%lu\t%lu\n",dmn_idx,srt_sz_t[dmn_idx],cnt_sz_t[dmn_idx]);
      (void)nco_inq_vardimid(nc_id,var_id,dmn_id);
      (void)fprintf(stdout,"Dimension sizes defined in and expected by netCDF output file:\ndmn_idx\tsz\n");
      for(int dmn_idx=0;dmn_idx<dmn_nbr;dmn_idx++){
