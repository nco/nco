 ...
 const char * const msg) /* I [sng] Supplemental error message */
{
  ...
  switch(rcd){
  case NC_EACCESS: (void)fprintf(stdout,"ERROR NC_EACCESS Access failure\nHINT: NC_EACCESS errors signify a problem receiving data from a DAP server. This can occur, e.g., when NCO requests more data than the server is configured to dispense. A workaround might be to request smaller chunks of data at one time.\n"); break;
