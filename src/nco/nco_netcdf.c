 ...
 const char * const msg) /* I [sng] Supplemental error message */
{
  ...
  case NC_EVARSIZE: (void)fprintf(stdout,"ERROR NC_EVARSIZE One or more variable sizes violate format constraints\nHINT: NC_EVARSIZE errors can occur when attempting to aggregate netCDF3 classic files together into outputs that exceed the capacity of the netCDF3 classic file format, e.g., a variable with size in excess of 2^31 B ~ 2.1 GB. In this case, try altering the output file type to netCDF3 classic with 64-bit offsets (with --64), to pnetCDF/CDF5 with 64-bit data (with -5), or to netCDF4 (with -4). For more details, see http://nco.sf.net/nco.html#fl_fmt\n"); break;
  } /* end switch */
