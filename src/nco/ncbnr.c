...
/* Usage:
   ncbnr -O in.nc in.nc foo.nc
   ncbnr -O -v mss_val in.nc in.nc foo.nc
   ncbnr -p /data/zender/tmp h0001.nc foo.nc
   ncbnr -p /data/zender/tmp -l /data/zender/tmp/rmt h0001.nc h0002.nc foo.nc
   ncbnr -p /ZENDER/tmp -l /data/zender/tmp/rmt h0001.nc h0002.nc foo.nc
   ncbnr -p /ZENDER/tmp -l /usr/tmp/zender h0001.nc h0002.nc foo.nc

   Test type conversion:
   ncks -O -C -v float_var in.nc foo1.nc
   ncrename -v float_var,double_var foo1.nc
   ncks -O -C -v double_var in.nc foo2.nc
   ncbnr -O -C -v double_var foo1.nc foo2.nc foo3.nc
   ncbnr -O -C -v double_var foo2.nc foo1.nc foo4.nc
   ncks -H -m foo1.nc
   ncks -H -m foo2.nc
   ncks -H -m foo3.nc
   ncks -H -m foo4.nc
...
main(int argc,char **argv)
{
 ...
 /* Start clock and save command line */ 
  ...
  /* The code for ncbnr() has been similar to ncea() (and ncra()) wherever possible
     The major differences occur where performance would otherwise suffer
     From now on, however, the binary-file and binary-operation nature of ncbnr()
     ...
     The other unique "feature" to know about ncbnr is that it tries too hard to save memory
     For example, ncbnr is a three file operator (input, input, output) but manages to get by 
     ...
     ncbnr overwrites variable structure from file_2 with structure for file_3 (output file)
